...
struct MaterialOverrideState
{
	...
	bool override_retro_reflection		  = false;
...
void ImGuiObjectsWindow::draw_global_objects_panel()
{
	...
	if (ImGui::CollapsingHeader("Base Layer"))
	{
		...
		if (ImGui::BeginTable("Table base layer", 2, ImGuiTableFlags_SizingFixedFit))
		{
			for (int row = 0; row < 6; row++)
			{
				...
				case 4:
					material_override_changed |= draw_material_override_line("Anisotropy rotation", override_state.override_anisotropy_rotation,
																			 material_override.anisotropy_rotation, 0.0f, 1.0f);
					break;

				case 5:
					material_override_changed |= draw_material_override_line("IOR", override_state.override_IOR, material_override.ior, 1.0f, 3.0f);
					if (material_override.ior < 1.0f ||
						material_override.ior > 3.0f && (material_override.do_glass_energy_compensation || material_override.do_specular_energy_compensation))
					{
						ImGui::SameLine();
						ImGuiRenderer::show_help_marker("Energy compensation behavior is undefined for IORs < 1.0f or IORs > 3.0f",
	...
	if (ImGui::CollapsingHeader("Specular layer"))
	{
		...
		if (ImGui::BeginTable("Table specular layer", 2, ImGuiTableFlags_SizingFixedFit))
		{
			for (int row = 0; row < 5; row++)
			{
				...
				case 3:
					material_override_changed |= draw_material_override_line("Specular tint strength", override_state.override_specular_tint_strength,
																			 material_override.specular_tint, 0.0f, 1.0f);
					break;

				case 4:
					material_override_changed |= draw_material_override_line("Specular darkening", override_state.override_specular_darkening,
																			 material_override.specular_darkening, 0.0f, 1.0f);
					break;
				}
			}
	...
	if (ImGui::CollapsingHeader("Metallic Layer"))
	{
		...
		if (ImGui::BeginTable("Table metallic layer", 2, ImGuiTableFlags_SizingFixedFit))
		{
			for (int row = 0; row < 9; row++)
			{
				...
				case 5:
					material_override_changed |= draw_material_override_line("F90 Falloff exponent", override_state.override_F90_falloff_exponent,
																			 material_override.metallic_F90_falloff_exponent, 0.5f, 5.0f);
				...
				case 6:
					material_override_changed |= draw_material_override_line("Retro reflection", override_state.override_retro_reflection,
																			 material_override.retro_reflection, 0.0f, 1.0f);
					break;

				case 7:
					ImGui::Dummy(ImVec2(0.0f, 20.0f));

					material_override_changed |= draw_material_override_line("Second roughness weight", override_state.override_second_roughness_weight,
																			 material_override.second_roughness_weight, 0.0f, 1.0f);
				...
				case 8:
					material_override_changed |= draw_material_override_line("Second roughness", override_state.override_second_roughness,
																			 material_override.second_roughness, 0.0f, 1.0f);
					break;
				}
			}
	...
	if (ImGui::CollapsingHeader("Coat Layer"))
	{
		...
		if (ImGui::BeginTable("Table coat layer", 2, ImGuiTableFlags_SizingFixedFit))
		{
			for (int row = 0; row < 10; row++)
			{
				...
				case 2:
					material_override_changed |= draw_material_override_line("Coat medium absorption", override_state.override_coat_medium_absorption,
																			 material_override.coat_medium_absorption);
					break;

				case 3:
					material_override_changed |= draw_material_override_line("Coat medium thickness", override_state.override_coat_medium_thickness,
																			 material_override.coat_medium_thickness, 0.0f, 15.0f);
					break;
				...
				case 8:
					material_override_changed |= draw_material_override_line("Coat anisotropy rotation", override_state.override_anisotropy_rotation,
																			 material_override.anisotropy_rotation, 0.0f, 1.0f);
					break;
	...
	if (ImGui::CollapsingHeader("Transmission Layer"))
	{
		...
		if (ImGui::BeginTable("Table transmission layer", 2, ImGuiTableFlags_SizingFixedFit))
		{
			for (int row = 0; row < 10; row++)
			{
				...
				case 1:
					material_override_changed |= draw_material_override_line("Diffuse transmission", override_state.override_diffuse_transmission,
																			 material_override.diffuse_transmission, 0.0f, 1.0f);
					break;

				case 2:
					material_override_changed |= draw_material_override_line("Specular transmission", override_state.override_transmission,
																			 material_override.specular_transmission, 0.0f, 1.0f);
					break;
				...
				case 4:
					material_override_changed |= draw_material_override_line("Absorption distance", override_state.override_absorption_distance,
																			 material_override.absorption_at_distance, 0.0f, 20.0f);
					break;
				...
				case 6:
					material_override_changed |= draw_material_override_line("Dispersion Abbe number", override_state.override_dispersion_abbe,
																			 material_override.dispersion_abbe_number, 9.0f, 91.0f);
				...
				case 7:
					material_override_changed |= draw_material_override_line("Dispersion scale", override_state.override_dispersion_scale,
																			 material_override.dispersion_scale, 0.0f, 1.0f);
					break;

				case 8:
					material_override_changed |= draw_material_override_line("Dielectric priority", override_state.override_dielectric_priority,
																			 material_override.dielectric_priority, 1, StackPriorityEntry::PRIORITY_MAXIMUM);
					break;
	...
	if (ImGui::CollapsingHeader("Thin-Film Layer"))
	{
		...
		if (ImGui::BeginTable("Table thin-film layer", 2, ImGuiTableFlags_SizingFixedFit))
		{
			for (int row = 0; row < 8; row++)
			{
				...
				case 2:
					material_override_changed |= draw_material_override_line("Thin film thickness", override_state.override_thin_film_thickness,
																			 material_override.thin_film_thickness, 1.0f, 2000.0f, "%.3f nm");
					break;
				...
				case 4:
					material_override_changed |= draw_material_override_line("Thin film hue shift", override_state.override_thin_film_hue_shift,
																			 material_override.thin_film_hue_shift_degrees, 0.0f, 360.0f);
					break;

				case 5:
					ImGui::Dummy(ImVec2(0.0f, 20.0f));

					material_override_changed |= draw_material_override_line("Override material IOR", override_state.override_thin_film_do_ior_override,
																			 material_override.thin_film_do_ior_override);
				...
				case 6:
					material_override_changed |= draw_material_override_line("Eta IOR override", override_state.override_thin_film_base_ior_override,
																			 material_override.thin_film_base_ior_override, 1.0f, 3.0f);
					ImGuiRenderer::show_help_marker("Overrides the eta parameter of the IOR of the base material. This is not physically based but allows for "
													"better artistic control.");
					break;

				case 7:
					material_override_changed |= draw_material_override_line("Kappa IOR override", override_state.override_thin_film_kappa_3,
																			 material_override.thin_film_kappa_3, 0.0f, 5.0f);
					ImGuiRenderer::show_help_marker("Overrides the kappa parameter (extinction coefficient) of the base material. This is not physically based "
													"but allows for better artistic control.");
	...
	if (ImGui::CollapsingHeader("Emission Properties"))
	{
		...
		if (ImGui::BeginTable("Table base layer", 2, ImGuiTableFlags_SizingFixedFit))
		{
			for (int row = 0; row < 3; row++)
			{
				...
				case 2:
					material_override_changed |= draw_material_override_line("Emission strength", override_state.override_emission_strength,
																			 material_override.emission_strength, 0.0f, 10.0f);
					break;
				}
			}
	...
	if (material_override_changed)
	{
		...
		apply_material_override(override_state.override_anisotropy_rotation, &CPUMaterial::anisotropy_rotation, material_override.anisotropy_rotation,
								overriden_materials);
		...
		apply_material_override(override_state.override_specular_tint_strength, &CPUMaterial::specular_tint, material_override.specular_tint,
								overriden_materials);
		apply_material_override(override_state.override_specular_darkening, &CPUMaterial::specular_darkening, material_override.specular_darkening,
								overriden_materials);
		...
		apply_material_override(override_state.override_F90_falloff_exponent, &CPUMaterial::metallic_F90_falloff_exponent,
								material_override.metallic_F90_falloff_exponent, overriden_materials);
		apply_material_override(override_state.override_second_roughness_weight, &CPUMaterial::second_roughness_weight,
								material_override.second_roughness_weight, overriden_materials);
		apply_material_override(override_state.override_second_roughness, &CPUMaterial::second_roughness, material_override.second_roughness,
								overriden_materials);
		...
		apply_material_override(override_state.override_coat_medium_absorption, &CPUMaterial::coat_medium_absorption, material_override.coat_medium_absorption,
								overriden_materials);
		apply_material_override(override_state.override_coat_medium_thickness, &CPUMaterial::coat_medium_thickness, material_override.coat_medium_thickness,
								overriden_materials);
		...
		apply_material_override(override_state.override_coat_anisotropy_rotation, &CPUMaterial::coat_anisotropy_rotation,
								material_override.coat_anisotropy_rotation, overriden_materials);
		...
		apply_material_override(override_state.override_transmission, &CPUMaterial::diffuse_transmission, material_override.diffuse_transmission,
								overriden_materials);
		apply_material_override(override_state.override_transmission, &CPUMaterial::specular_transmission, material_override.specular_transmission,
								overriden_materials);
		apply_material_override(override_state.override_IOR, &CPUMaterial::ior, material_override.ior, overriden_materials);
		apply_material_override(override_state.override_absorption_distance, &CPUMaterial::absorption_at_distance, material_override.absorption_at_distance,
								overriden_materials);
		apply_material_override(override_state.override_absorption_color, &CPUMaterial::absorption_color, material_override.absorption_color,
								overriden_materials);
		apply_material_override(override_state.override_dispersion_abbe, &CPUMaterial::dispersion_abbe_number, material_override.dispersion_abbe_number,
								overriden_materials);
		apply_material_override(override_state.override_dispersion_scale, &CPUMaterial::dispersion_scale, material_override.dispersion_scale,
								overriden_materials);
		apply_material_override(override_state.override_dielectric_priority, &CPUMaterial::dielectric_priority, material_override.dielectric_priority,
								overriden_materials);
		...
		apply_material_override(override_state.override_thin_film_thickness, &CPUMaterial::thin_film_thickness, material_override.thin_film_thickness,
								overriden_materials);
		...
		apply_material_override(override_state.override_thin_film_do_ior_override, &CPUMaterial::thin_film_do_ior_override,
								material_override.thin_film_do_ior_override, overriden_materials);
		apply_material_override(override_state.override_thin_film_base_ior_override, &CPUMaterial::thin_film_base_ior_override,
								material_override.thin_film_base_ior_override, overriden_materials);
		apply_material_override(override_state.override_thin_film_kappa_3, &CPUMaterial::thin_film_kappa_3, material_override.thin_film_kappa_3,
								overriden_materials);
		apply_material_override(override_state.override_thin_film_hue_shift, &CPUMaterial::thin_film_hue_shift_degrees,
								material_override.thin_film_hue_shift_degrees, overriden_materials);
		...
		apply_material_override(override_state.override_emission_strength, &CPUMaterial::emission_strength, material_override.emission_strength,
								overriden_materials);
...
bool ImGuiObjectsWindow::draw_material_editor(CPUMaterial& material,
											  ...
											  bool& out_emission_changed)
{
	...
	if (ImGui::CollapsingHeader("Specular Layer"))
	{
		...
		if (material.do_specular_energy_compensation &&
			kernel_options->get_macro_value(GPUKernelCompilerOptions::PRINCIPLED_BSDF_DO_ENERGY_COMPENSATION) == KERNEL_OPTION_FALSE)
		{
			ImGui::Text("Warning: ");
			ImGuiRenderer::show_help_marker("Energy compensation is globally disabled. This material option will have no effect.\n"
											"Energy compensation can be globally enabled in \"Settings\" --> \"Sampling\" --> \"Materials\"",
											ImVec4(1.0f, 1.0f, 0.0f, 1.0f));
		}
		else if (material.do_specular_energy_compensation &&
				 kernel_options->get_macro_value(GPUKernelCompilerOptions::PRINCIPLED_BSDF_DO_SPECULAR_ENERGY_COMPENSATION) == KERNEL_OPTION_FALSE)
		{
			ImGui::Text("Warning: ");
			ImGuiRenderer::show_help_marker("Energy compensation is globally disabled for the glossy layer (specular/diffuse). This material option will have "
											"no effect.\n"
											"Energy compensation can be enabled in \"Settings\" --> \"Sampling\" --> \"Materials\"",
											ImVec4(1.0f, 1.0f, 0.0f, 1.0f));
		}
	...
	if (ImGui::CollapsingHeader("Metallic Layer"))
	{
		...
		ImGui::Dummy(ImVec2(0.0f, 20.0f));
		material_changed |= ImGui::SliderFloat("Retro-reflection", &material.retro_reflection, 0.0f, 1.0f);
		...
		else if (material.do_metallic_energy_compensation &&
				 kernel_options->get_macro_value(GPUKernelCompilerOptions::PRINCIPLED_BSDF_DO_METALLIC_ENERGY_COMPENSATION) == KERNEL_OPTION_FALSE)
		{
			ImGui::Text("Warning: ");
	...
	if (ImGui::CollapsingHeader("Coat Layer"))
	{
		...
		if (material.do_coat_energy_compensation &&
			kernel_options->get_macro_value(GPUKernelCompilerOptions::PRINCIPLED_BSDF_DO_ENERGY_COMPENSATION) == KERNEL_OPTION_FALSE)
		{
			ImGui::Text("Warning: ");
			ImGuiRenderer::show_help_marker("Energy compensation is globally disabled. This material option will have no effect.\n"
		...
		else if (material.do_coat_energy_compensation &&
				 kernel_options->get_macro_value(GPUKernelCompilerOptions::PRINCIPLED_BSDF_DO_CLEARCOAT_ENERGY_COMPENSATION) == KERNEL_OPTION_FALSE)
		{
			ImGui::Text("Warning: ");
	...
	if (ImGui::CollapsingHeader("Transmission Layer"))
	{
		...
		else if (material.do_glass_energy_compensation &&
				 kernel_options->get_macro_value(GPUKernelCompilerOptions::PRINCIPLED_BSDF_DO_GLASS_ENERGY_COMPENSATION) == KERNEL_OPTION_FALSE)
		{
			ImGui::Text("Warning: ");
	...
	if (ImGui::CollapsingHeader("Thin-Film Layer"))
	{
		...
		ImGuiRenderer::show_help_marker("Overrides the eta parameter of the IOR of the base material. This is not physically based but allows for better "
										"artistic control.");
		material_changed |= ImGui::SliderFloat("Kappa IOR override", &material.thin_film_kappa_3, 0.0f, 5.0f);
		ImGuiRenderer::show_help_marker("Overrides the kappa parameter (extinction coefficient) of the base material. This is not physically based but allows "
										"for better artistic control.");
		ImGui::EndDisabled();
																	   ...
																	   const std::string& filter_string) const
{
	...
	{
		auto found = std::search(haystack.begin(), haystack.end(), needle.begin(), needle.end(),
