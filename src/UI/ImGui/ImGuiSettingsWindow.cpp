...
void ImGuiSettingsWindow::draw_render_settings_panel()
{
	...
	std::vector<const char*> preset_items = { "Default", "Reference path-tracer", "MIS NEE Path Tracer", "RIS NEE Path Tracer", "ReSTIR GI" };
...
void ImGuiSettingsWindow::draw_render_stopping_conditions_panel()
{
	...
	if (ImGui::CollapsingHeader("Render stopping condition"))
	{
		ImGui::TreePush("Stopping condition tree");
		{
			...
			if (checkbox_changed |= ImGui::Checkbox("Render only last sample", &last_sample_only))
				m_render_window->set_render_dirty(true);
			...
			ImGui::BeginDisabled(render_settings.adaptive_sampling_enabled() || !render_settings.use_pixel_stop_noise_threshold);
			{
				...
				if (render_settings.adaptive_sampling_enabled())
...
void ImGuiSettingsWindow::draw_sampling_panel()
{
	...
	if (ImGui::CollapsingHeader("Sampling"))
	{
		...
		if (ImGui::CollapsingHeader("Adaptive sampling"))
		{
			...
			if (ImGui::CollapsingHeader("Per-pixel adaptive sampling"))
			{
				ImGui::TreePush("Per-pixel adaptive sampling tree");
				...
				if (ImGui::Checkbox("Enable per-pixel adaptive sampling", (bool*)&render_settings.enable_adaptive_sampling))
				{
				...
				if (render_settings.enable_adaptive_sampling)
				{
					...
					if (global_kernel_options->get_macro_value(GPUKernelCompilerOptions::PATH_SAMPLING_STRATEGY) == PATH_SAMPLING_RESTIR_GI)
					{
						ImGuiRenderer::add_warning(
							"Adaptive sampling may not be efficient when used with ReSTIR. This is because "
					...
					const char* adaptive_sampling_debug_view_tooltips[] = {
						...
						"and "
						"higher sample counts to its last color.",
					...
					if (ImGuiRenderer::ComboWithTooltips(
							"Debug view##adaptive-sampling", &render_data.display_post_process_settings.adaptive_sampling_display_view,
			...
			if (ImGui::CollapsingHeader("Hierarchical adaptive sampling"))
			{
				ImGui::TreePush("Hierarchical adaptive sampling tree");
				...
				ImGuiRenderer::show_help_marker(
					"Jeffery 2019 hierarchical adaptive sampling. The full hierarchy is rebuilt periodically, so completed regions may reactivate.\n\n"
					"Sample subsets and low-resolution interaction rendering temporarily bypass this mode.");
				if (!render_settings.accumulate)
					ImGuiRenderer::add_tooltip("Cannot use adaptive sampling when accumulation is not on.");
				if (global_kernel_options->get_macro_value(GPUKernelCompilerOptions::PATH_SAMPLING_STRATEGY) == PATH_SAMPLING_RESTIR_GI)
				{
					ImGuiRenderer::add_warning(
						...
						"ReSTIR is reused.\n"
						"Thus, these sample are not reused to avoid bias but this then drastically reduces the efficiency of ReSTIR and the "
						"overall setup becomes inefficient.");
				}

				ImGui::BeginDisabled(!render_settings.enable_hierarchical_adaptive_sampling);
				if (ImGui::InputInt("Minimum samples##hierarchical-adaptive", &render_settings.adaptive_sampling_min_samples))
				{
					render_settings.adaptive_sampling_min_samples = std::max(2, render_settings.adaptive_sampling_min_samples);
				...
				{
					render_settings.hierarchical_adaptive_sampling_minimum_cell_extent =
						std::max(1.0f, render_settings.hierarchical_adaptive_sampling_minimum_cell_extent);
				...
				if (ImGui::InputInt("Maximum hierarchy nodes", &render_settings.hierarchical_adaptive_sampling_max_node_count))
				{
					render_settings.hierarchical_adaptive_sampling_max_node_count = std::max(1, render_settings.hierarchical_adaptive_sampling_max_node_count);

					m_render_window->set_render_dirty(true);
				}
				ImGuiRenderer::show_help_marker("If this bounded capacity is exhausted, affected regions remain active and are safely oversampled.");
			...
			ImGui::Dummy(ImVec2(0.0f, 20.0f));
			ImGui::TreePop(); // Adaptive sampling tree
		}
		...
		if (ImGui::CollapsingHeader("Emissive geometry sampling"))
		{
			...
			bool using_learning_to_cluster_or_nisml =
				DIRECT_LIGHT_NEE_IS_LEARNING_TO_CLUSTER(global_kernel_options->get_macro_value(GPUKernelCompilerOptions::DIRECT_LIGHT_NEE_ESTIMATOR)) ||
			...
			const char* items_nee_estimators[]	  = { "- No direct light sampling",
													  ...
													  "- Learning to cluster SG light tree + MIS" };
			const char* tooltips_nee_estimators[] = {
				...
				"light or BSDF candidates can be controlled.",
				...
				"account. Not all BSDF lobe configurations are supported.",

				"Implementation of [Neural Importance Sampling for Many Lights, Figueiredo et al. 2025]. Uses an MLP to learn sampling probabilities on "
				"clusters of a lightcut in the spherical gaussian light tree",
				...
				"Combines learning-to-cluster light sampling with the path continuation BSDF sample using deferred MIS. Learns MIS-weighted cluster "
				"contributions.",
			};
					...
					items_nee_estimators, IM_ARRAYSIZE(items_nee_estimators), tooltips_nee_estimators, disabled_nee_estimator_items))
			{
				...
				else if (DIRECT_LIGHT_NEE_IS_LEARNING_TO_CLUSTER(nee_estimator))
				{
					...
					m_renderer->get_light_tree_sg_sampling_data_structure().set_spatial_lobe_count(
						LightTreeSGBuilderOptions::LIGHT_TREE_SG_DEFAULT_SPATIAL_LOBE_COUNT);
...
void ImGuiSettingsWindow::draw_ReGIR_settings_panel()
{
	...
	if (ImGui::CollapsingHeader("ReGIR Settings") && regir_render_pass)
	{
		...
		ImGui::Dummy(ImVec2(0.0f, 20.0f));
		ReGIRVRAMUsage vram_usage = regir_render_pass->get_vram_usage_breakdown();
		ImGui::Text("VRAM Usage: %.3fMB (avg. %.1fB per cell)", vram_usage.get_total_bytes() / 1000000.0f,
					vram_usage.get_total_bytes() /
		...
		snprintf(tooltip_buffer.data(), 2048,
				 ...
				 vram_usage.primary_hits.get_reservoirs_bytes() / 1000000.0f, vram_usage.primary_hits.base_reservoirs / 1000000.0f,
				 vram_usage.primary_hits.spatial_reuse_reservoirs / 1000000.0f, vram_usage.primary_hits.correlation_reduction / 1000000.0f,
				 vram_usage.primary_hits.cell_world_data / 1000000.0f, vram_usage.primary_hits.async_compute / 1000000.0f,
				 vram_usage.primary_hits.ris_pre_integration / 1000000.0f, vram_usage.primary_hits.light_distributions / 1000000.0f,

				 vram_usage.secondary_hits.get_reservoirs_bytes() / 1000000.0f, vram_usage.secondary_hits.base_reservoirs / 1000000.0f,
				 vram_usage.secondary_hits.spatial_reuse_reservoirs / 1000000.0f, vram_usage.secondary_hits.cell_world_data / 1000000.0f,
				 vram_usage.secondary_hits.async_compute / 1000000.0f, vram_usage.secondary_hits.ris_pre_integration / 1000000.0f,
				 vram_usage.secondary_hits.light_distributions / 1000000.0f);
...
void ImGuiSettingsWindow::draw_light_tree_SG_settings_panel()
{
	...
	bool not_using_raw_sg_tree = using_neural_many_lights;
	...
	if (ImGui::CollapsingHeader("Light tree SG settings"))
	{
		...
		ImGui::PopStyleVar();
		{
			...
			if (not_using_raw_sg_tree)
				ImGuiRenderer::add_tooltip("Tree cut sampling is disabled because of currently using neural many lights. This "
			...
			if (not_using_raw_sg_tree)
				ImGuiRenderer::add_tooltip("Adaptive splitting is only available when using the SG tree itself for sampling, i.e. not when using a technique "
										   "on top of the SG tree (neural many lights)");
			ImGui::EndDisabled(); // not_using_raw_sg_tree
...
void ImGuiSettingsWindow::draw_illumination_aware_kd_tree_panel()
{
	...
	if (using_illumination_aware_kd_tree)
	{
		...
		{
			ImGui::TreePush("Illumination aware KD-tree tree");
			...
			if (illumination_aware_kd_tree_render_pass)
			{
				...
				static int nodes_buffer_capacity = illumination_aware_kd_tree_render_pass->get_current_node_buffer_capacity();
				ImGui::InputInt("Nodes buffer capacity", &nodes_buffer_capacity);

				if (nodes_buffer_capacity != illumination_aware_kd_tree_render_pass->get_current_node_buffer_capacity())
				{
					ImGui::TreePush("Apply button illumination-aware KD-tree nodes buffer capacity");
					if (ImGui::Button("Apply"))
					{
						illumination_aware_kd_tree_render_pass->get_current_node_buffer_capacity() = nodes_buffer_capacity;
						illumination_aware_kd_tree_render_pass->mark_buffers_need_reallocation();
	...
	if (using_learning_to_cluster && illumination_aware_kd_tree_render_pass)
	{
		if (ImGui::CollapsingHeader("Learning to cluster many lights"))
		{
			...
			if (ImGui::SliderInt("Refinement rounds per SPP", &illumination_aware_kd_tree_render_pass->get_lightcut_refinement_rounds_per_SPP(), 1, 8))
				m_render_window->set_render_dirty(true);
			ImGuiRenderer::show_help_marker("Number of light-cut refinement rounds per SPP. Trades perf for better lightcut refinement quality.");
			if (ImGui::SliderInt("Refinement rounds SPP limit", &illumination_aware_kd_tree_render_pass->get_lightcut_refinement_rounds_SPP_limit(), 0, 16))
				m_render_window->set_render_dirty(true);
			ImGuiRenderer::show_help_marker(
				"After that many samples rendered, the lightcut refinement rounds per SPP will be limited to 1. 0 disables the limit.");
			ImGui::Dummy(ImVec2(0.0f, 20.0f));
			...
			static int initial_lightcut_size = static_cast<int>(render_data.kd_tree_device.learning_to_cluster.user_settings.initial_lightcut_size);
			static int maximum_lightcut_size = global_kernel_options->get_macro_value(GPUKernelCompilerOptions::LEARNING_TO_CLUSTER_MAXIMUM_LIGHT_CUT_SIZE);
			ImGui::SliderInt("Initial light cut size", &initial_lightcut_size, 1, maximum_lightcut_size, "%d", ImGuiSliderFlags_AlwaysClamp);
			ImGuiRenderer::show_help_marker("Initial number of SG light-tree clusters used by LTC before adaptive refinement.");

			ImGui::SliderInt("Maximum light cut size", &maximum_lightcut_size, initial_lightcut_size, 1024, "%d", ImGuiSliderFlags_AlwaysClamp);
			ImGuiRenderer::show_help_marker("Maximum number of SG light-tree clusters in the LTC cut.");

			if (maximum_lightcut_size != global_kernel_options->get_macro_value(GPUKernelCompilerOptions::LEARNING_TO_CLUSTER_MAXIMUM_LIGHT_CUT_SIZE))
			{
				...
				{
					global_kernel_options->set_macro_value(GPUKernelCompilerOptions::LEARNING_TO_CLUSTER_MAXIMUM_LIGHT_CUT_SIZE, maximum_lightcut_size);
					render_data.kd_tree_device.learning_to_cluster.user_settings.initial_lightcut_size = initial_lightcut_size;
			...
			else if (initial_lightcut_size != static_cast<int>(render_data.kd_tree_device.learning_to_cluster.user_settings.initial_lightcut_size))
			{
			...
			const char* debug_view_items[]	  = { "- No debug",
												  "- Light-cut size heatmap",
												  "- Cell/normal-face colors solid",
												  "- Cell/normal-face colors outlines",
												  "- Cell/normal-face/mesh-ID colors solid",
												  "- Cell/normal-face/mesh-ID colors outlines" };
			const char* debug_view_tooltips[] = {
				...
				"Displays one deterministic random color for each visible KD-tree cell, surface-normal bucket, and mesh-ID combination.",
				"Displays outlines where neighboring pixels belong to different KD-tree cell, surface-normal bucket, or mesh-ID combinations."
			};
			if (ImGuiRenderer::ComboWithTooltips(
	...
	if (using_neural_many_lights)
	{
		if (ImGui::CollapsingHeader("Neural importance sampling of many lights"))
		{
			...
			if (nisml_render_pass)
			{
				...
				ImGui::Dummy(ImVec2(0.0f, 20.0f));
				ImGui::PushStyleVar(ImGuiStyleVar_SeparatorTextBorderSize, 5.0f);
				ImGui::SeparatorText("SG importance caches");
				ImGui::PopStyleVar();
				static bool use_sg_importance_caches =
					global_kernel_options->get_macro_value(GPUKernelCompilerOptions::NISML_USE_SG_IMPORTANCES_KD_TREE_CACHES) == KERNEL_OPTION_TRUE;
				if (ImGui::Checkbox("Use SG importance caches", &use_sg_importance_caches))
				{
					global_kernel_options->set_macro_value(GPUKernelCompilerOptions::NISML_USE_SG_IMPORTANCES_KD_TREE_CACHES,
														   use_sg_importance_caches ? KERNEL_OPTION_TRUE : KERNEL_OPTION_FALSE);
				...
				if (use_sg_importance_caches)
				{
					...
					if (illumination_aware_kd_tree_render_pass)
					{
						if (ImGui::InputInt("Representatives per NISML cache entry##nisml",
											&illumination_aware_kd_tree_render_pass->get_nisml_representative_capacity()))
						{
							illumination_aware_kd_tree_render_pass->get_nisml_representative_capacity() =
								std::max(illumination_aware_kd_tree_render_pass->get_nisml_representative_capacity(), 1);
							illumination_aware_kd_tree_render_pass->mark_buffers_need_reallocation();
							m_render_window->set_render_dirty(true);
						}
						...
						ImGuiRenderer::show_help_marker("Reserved GPU memory budget for the fixed-capacity NISML cached SG-cluster importances hash table.");

						if (ImGui::InputInt("NISML SG importance hash normal precision##nisml",
											&illumination_aware_kd_tree_render_pass->get_nisml_hash_normal_precision()))
						{
							illumination_aware_kd_tree_render_pass->mark_buffers_need_reallocation();

							m_render_window->set_render_dirty(true);
						}
						ImGuiRenderer::show_help_marker("Normal quantization precision used in NISML SG-importance cache keys.");
					}
				}
...
void ImGuiSettingsWindow::draw_ReSTIR_neighbor_heuristics_panel()
{
	...
	// For ReSTIR GI only
	static bool use_neighbor_sample_point_roughness_heuristic_backup = false;
	static bool use_jacobian_heuristic_backup						 = false;
...
{
	HIPRTRenderSettings& render_settings							  = m_renderer->get_render_settings();
	ReSTIRCommonTemporalPassSettings& restir_common_temporal_settings = ReSTIRVariant == ReSTIR_VARIANT_GI
																			? m_renderer->get_render_settings().restir_gi_settings.common_temporal_pass
	...
	if (ImGui::CollapsingHeader("Temporal Reuse Pass"))
	{
		...
		ImGui::TreePush("ReSTIR - Temporal Reuse Pass Tree");
		{
			...
			if (restir_common_temporal_settings.do_temporal_reuse_pass)
			{
				...
				ImGui::Dummy(ImVec2(0.0f, 20.0f));
				int& m_cap = ReSTIRVariant == ReSTIR_VARIANT_GI ? m_renderer->get_render_settings().restir_gi_settings.m_cap
	...
	if (ImGui::CollapsingHeader("Spatial Reuse Pass"))
	{
		...
		ImGui::TreePush("ReSTIR - Spatial Reuse Pass Tree");
		{
			...
			if (restir_settings.do_spatial_reuse_pass)
			{
				...
				{
					global_kernel_options->set_macro_value(ReSTIRVariant == ReSTIR_VARIANT_GI
...
void ImGuiSettingsWindow::draw_ReSTIR_bias_correction_panel()
{
	...
	if (ImGui::CollapsingHeader("MIS Weights"))
	{
		...
		std::vector<const char*> tooltips = {
			"Very simple biased weights as described in the 2020 ReSTIR paper(Eq. 6).\n"
		...
		{
			tooltips.push_back("Not implemented for ReSTIR GI");
			tooltips.push_back("Not implemented for ReSTIR GI");
		}
		...
		int* mis_weights_type_option_pointer = global_kernel_options->get_raw_pointer_to_macro_value(
			ReSTIRVariant == ReSTIR_VARIANT_GI ? GPUKernelCompilerOptions::RESTIR_GI_MIS_WEIGHTS_TYPE : GPUKernelCompilerOptions::RESTIR_PT_MIS_WEIGHTS_TYPE);
		...
		if constexpr (ReSTIRVariant == ReSTIR_VARIANT_GI)
		{
			...
			if (ImGui::Checkbox("Use visibility in MIS weights", &mis_weights_use_visibility))
			{
				int* bias_correction_use_visibility_option_pointer =
					global_kernel_options->get_raw_pointer_to_macro_value(GPUKernelCompilerOptions::RESTIR_GI_MIS_WEIGHTS_USE_VISIBILITY);
