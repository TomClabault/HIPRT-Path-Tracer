...
// - Can we cache some things in ReGIR to avoid repopulating / re integrating etc... if we can actually still keep the pre pop, pre int�gration etc... Results of last time? Enabling/disabling GMoN for example resets the render but doesn't invalidate the pre-population / etc...
...
//		Should probably rearchitecture the thing to run after ReGIR has sampled lights so that we have a better idea of where the lights are and accumulate visibility information for the relevant lights
//		
...
// - We should have a mean of updating the light distributions at each cell because NEE++ is going to get better with time so we want to update the distributions
