...
void ReSTIRGIRenderPass::resize(unsigned int new_width, unsigned int new_height)
{
	...
	ReSTIRRenderPassCommon::resize_directional_reuse_buffers<ReSTIR_VARIANT_GI, false>(m_renderer, new_width, new_height, m_per_pixel_spatial_reuse_radius,
...
bool ReSTIRGIRenderPass::pre_render_update(float delta_time)
{
	...
	if (is_render_pass_used())
	{
		...
		render_data_invalidated |= ReSTIRRenderPassCommon::pre_render_update_directional_reuse_buffers<ReSTIR_VARIANT_GI, false>(
	...
	else
	{
		...
		render_data_invalidated |= ReSTIRRenderPassCommon::free_directional_reuse_buffers<ReSTIR_VARIANT_GI, false>(
...
void ReSTIRGIRenderPass::compute_optimal_spatial_reuse_radii(HIPRTRenderData& render_data)
{
	...
	if (accumulating && first_frame && not_interacting && using_adaptive_directional_spatial_reuse)
	{
		...
		unsigned int* per_pixel_spatial_reuse_direction_mask_u =
			m_per_pixel_spatial_reuse_direction_mask_u.size() > 0 ? m_per_pixel_spatial_reuse_direction_mask_u.get_device_pointer() : nullptr;
		unsigned long long int* per_pixel_spatial_reuse_direction_mask_ull =
			m_per_pixel_spatial_reuse_direction_mask_ull.size() > 0 ? m_per_pixel_spatial_reuse_direction_mask_ull.get_device_pointer() : nullptr;
...
void ReSTIRGIRenderPass::update_render_data()
{
	...
	if (is_render_pass_used())
	{
		...
		ReSTIRRenderPassCommon::update_render_data_common_buffers<ReSTIR_VARIANT_GI, false>(
