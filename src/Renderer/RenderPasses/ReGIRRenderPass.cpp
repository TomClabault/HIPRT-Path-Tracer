...
bool ReGIRRenderPass::pre_render_update(float delta_time)
{
	...
	if (is_render_pass_used())
	{
		...
		if (m_grid_cells_alive_buffer.size() != render_data.render_settings.regir_settings.get_total_number_of_cells())
		{
			m_grid_cells_alive_staging_buffer.resize(render_data.render_settings.regir_settings.get_total_number_of_cells());
			m_grid_cells_alive_buffer.resize(render_data.render_settings.regir_settings.get_total_number_of_cells());
			m_grid_cells_alive_list_buffer.resize(render_data.render_settings.regir_settings.get_total_number_of_cells());
			...
			std::vector<unsigned int> init_data_inactive(render_data.render_settings.regir_settings.get_total_number_of_cells(), 0u);
			m_grid_cells_alive_staging_buffer.upload_data(init_data_inactive);

			// Initializing all the cells to alive
			std::vector<unsigned char> init_data_alive(render_data.render_settings.regir_settings.get_total_number_of_cells(), 1u);
			m_grid_cells_alive_buffer.upload_data(init_data_alive);

			// All cells are alive at the beginning of the render
			unsigned int all_cells = render_data.render_settings.regir_settings.get_total_number_of_cells();
		...
		if (m_distance_to_center_buffer.size() != render_data.render_settings.regir_settings.get_total_number_of_cells())
		{
			m_distance_to_center_buffer.resize(render_data.render_settings.regir_settings.get_total_number_of_cells());
			m_representative_points_buffer.resize(render_data.render_settings.regir_settings.get_total_number_of_cells());
			m_representative_normals_buffer.resize(render_data.render_settings.regir_settings.get_total_number_of_cells());
			m_representative_primitive_buffer.resize(render_data.render_settings.regir_settings.get_total_number_of_cells());
...
void ReGIRRenderPass::reset(bool reset_by_camera_movement)
{
	...
	if (m_grid_cells_alive_buffer.size() > 0)
	{
		...
		std::vector<unsigned char> init_data_alive(render_data.render_settings.regir_settings.get_total_number_of_cells(), 1);
		m_grid_cells_alive_buffer.upload_data(init_data_alive);
