...
ReGIRRenderPass::ReGIRRenderPass(GPURenderer* renderer) : RenderPass(renderer, ReGIRRenderPass::REGIR_RENDER_PASS_NAME)
{
	...
	m_kernels[ReGIRRenderPass::REGIR_GRID_FILL_TEMPORAL_REUSE_FIRST_HITS_KERNEL_ID]->synchronize_options_with(global_compiler_options,
	...
	m_kernels[ReGIRRenderPass::REGIR_GRID_FILL_TEMPORAL_REUSE_SECONDARY_HITS_KERNEL_ID]->synchronize_options_with(global_compiler_options,
	...
	m_kernels[ReGIRRenderPass::REGIR_SPATIAL_REUSE_FIRST_HITS_KERNEL_ID]->synchronize_options_with(global_compiler_options,
																								   GPURenderer::KERNEL_OPTIONS_NOT_SYNCHRONIZED);
	...
	m_kernels[ReGIRRenderPass::REGIR_PRE_INTEGRATION_KERNEL_ID]->synchronize_options_with(global_compiler_options,
																						  GPURenderer::KERNEL_OPTIONS_NOT_SYNCHRONIZED);
	...
	m_kernels[ReGIRRenderPass::REGIR_GRID_FILL_TEMPORAL_REUSE_FOR_PRE_INTEGRATION_KERNEL_ID]->synchronize_options_with(global_compiler_options,
																													   options_not_synchronized);
...
bool ReGIRRenderPass::launch_cell_light_distributions_compute_and_sort_internal(HIPRTRenderData& render_data, bool primary_hit, bool compute_only_sizes)
{
	...
	std::vector<unsigned int> grid_cell_alive_list =
							m_hash_grid_storage.get_hash_cell_data_soa(primary_hit)
													.m_hash_cell_data.template get_buffer<ReGIRHashCellDataSoAHostBuffers::REGIR_HASH_CELLS_ALIVE_LIST>()
													.download_data();
	std::vector<unsigned short int> light_distribution_sizes =
							!compute_only_sizes ? m_hash_grid_storage.get_cell_light_distributions(primary_hit)
																			.soa
																			.download_buffer<ReGIRCellsLightDistributionsSoAHostBuffers::
																													 REGIR_CELLS_LIGHT_DISTRIBUTIONS_SIZES>()
												: std::vector<unsigned short int>(m_hash_grid_storage.get_total_number_of_cells(primary_hit));
	std::vector<unsigned int> light_distribution_offsets =
							!compute_only_sizes ? m_hash_grid_storage.get_cell_light_distributions(primary_hit)
												...
												: std::vector<unsigned int>(m_hash_grid_storage.get_total_number_of_cells(primary_hit),
																			ReGIRCellsLightDistributionsSoADevice::NO_AVAILABLE_LIGHT_DISTRIBUTION);
	...
	std::vector<unsigned long long int> meshes_indices_staging(
							m_hash_grid_storage.get_cell_light_distributions(primary_hit)
													...
													.size());
	std::vector<unsigned short int> CDF_staging_u16(
							m_hash_grid_storage.get_cell_light_distributions(primary_hit)
