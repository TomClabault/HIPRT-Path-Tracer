...
bool ReGIRRenderPass::pre_frame_render_update(float delta_time)
{
	...
	if (is_render_pass_used(*m_compiler_options))
	{
		bool storage_updated = m_hash_grid_storage.pre_frame_render_update(render_data);
		if (storage_updated)
...
bool ReGIRRenderPass::launch_cell_light_distributions_precomputation_internal(HIPRTRenderData& render_data, bool primary_hit)
{
	...
	std::cout << "Light distribution precomputation for " << (primary_hit ? "primary hits" : "secondary hits")
...
bool ReGIRRenderPass::launch_cell_light_distributions_compute_and_sort_internal(HIPRTRenderData& render_data, bool primary_hit, bool compute_only_sizes)
{
	...
	m_light_distributions_build_kernels.m_radix_sort.resize(scratch_buffer_size);
	m_light_distributions_build_kernels.sum_reduction_all_cell_contributions.resize(scratch_buffer_size);
	m_light_distributions_build_kernels.sum_reduction_light_distrib_size_cell_contributions.resize(scratch_buffer_size);
	m_light_distributions_build_kernels.m_compute_light_distributions_CDFs_prefix_scan.resize(scratch_buffer_size);
	...
	// clang-format off
	unsigned int* grid_cell_alive_list_GPU_buffer_pointer = m_hash_grid_storage.get_hash_cell_data_soa(primary_hit).m_hash_cell_data.
	...
	if (compute_only_sizes)
	{
		...
		m_light_distributions_build_kernels.m_memset_ushort_kernel.launch_asynchronous(1024, 1, distrib_sizes_buffer_size, 1, memset_args,
																					   m_renderer->get_main_stream());
	}
	...
	if (compute_only_sizes)
	{
		...
		std::vector<unsigned int> mesh_indices_offsets = std::vector<unsigned int>(m_hash_grid_storage.get_total_number_of_cells(primary_hit));
		std::vector<unsigned int> light_distribution_offsets = std::vector<unsigned int>(m_hash_grid_storage.get_total_number_of_cells(primary_hit), ReGIRCellsLightDistributionsSoADevice::NO_AVAILABLE_LIGHT_DISTRIBUTION);
		std::vector<unsigned short int> light_distribution_sizes = light_distribution_sizes_buffer_GPU.download_data();
		// clang-format on
	...
	m_light_distributions_build_kernels.m_radix_sort.free();
	m_light_distributions_build_kernels.sum_reduction_all_cell_contributions.free();
	m_light_distributions_build_kernels.sum_reduction_light_distrib_size_cell_contributions.free();
	m_light_distributions_build_kernels.m_compute_light_distributions_CDFs_prefix_scan.free();
...
float ReGIRRenderPass::get_full_frame_time()
{
	...
	for (auto& name_to_kernel : get_all_kernels())
	{
		if (name_to_kernel.first == ReGIRRenderPass::REGIR_GRID_PRE_POPULATE || name_to_kernel.first == ReGIRRenderPass::REGIR_REHASH_KERNEL_ID)
