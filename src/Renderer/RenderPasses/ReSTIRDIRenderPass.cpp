...
bool ReSTIRDIRenderPass::pre_render_update(float delta_time)
{
	...
	{
		// ReSTIR DI enabled
		bool initial_candidates_reservoir_needs_resize = m_initial_candidates_reservoirs.get_element_count() == 0;
		bool spatial_output_1_needs_resize = m_spatial_output_reservoirs_1.get_element_count() == 0;
		bool spatial_output_2_needs_resize = m_spatial_output_reservoirs_2.get_element_count() == 0;
		...
		if (m_renderer->get_global_compiler_options()->get_macro_value(GPUKernelCompilerOptions::RESTIR_DI_DO_LIGHTS_PRESAMPLING) == KERNEL_OPTION_TRUE)
		{
			...
			bool presampled_lights_needs_allocation = m_presampled_lights_buffer.get_element_count() != presampled_light_count;
		...
		render_data_invalidated |= ReSTIRRenderPassCommon::pre_render_update_common_buffers<false>(*m_render_data, m_renderer,
	...
	else
	{
		...
		render_data_invalidated |= ReSTIRRenderPassCommon::free_common_buffers<false>(
			m_per_pixel_spatial_reuse_radius, 
			m_per_pixel_spatial_reuse_direction_mask_u,
			m_per_pixel_spatial_reuse_direction_mask_ull,
			m_spatial_reuse_statistics_hit_hits,
...
void ReSTIRDIRenderPass::resize(unsigned int new_width, unsigned int new_height)
{
	...
	ReSTIRRenderPassCommon::resize_common_buffers<false>(m_renderer, new_width, new_height, 
		m_per_pixel_spatial_reuse_radius, 
		m_per_pixel_spatial_reuse_direction_mask_u, 
		m_per_pixel_spatial_reuse_direction_mask_ull, 
		m_decoupled_shading_reuse_buffer);
