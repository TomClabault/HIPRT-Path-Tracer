...
public:
	...
	static const std::string LEARNING_TO_CLUSTER_REPLAY_STATISTICS_KERNEL_ID;
	static const std::string LEARNING_TO_CLUSTER_REFINE_LIGHTCUTS_KERNEL_ID;
	static const std::string LEARNING_TO_CLUSTER_REPLAY_Q_REWARDS_KERNEL_ID;
...
public:
	...
	int& get_lightcut_refinement_rounds_SPP_limit();
	...
	unsigned int get_current_allocated_lightcut_count() const;
...
private:
	...
	int m_lightcut_refinement_rounds_per_SPP = 2;
	// After that many samples rendered, the lightcut refinement rounds per SPP will be limited to 1
	int m_lightcut_refinement_rounds_SPP_limit = 8;
	...
	int m_learning_to_cluster_learning_spp		= 12800;
