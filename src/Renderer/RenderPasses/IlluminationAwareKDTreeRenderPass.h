...
struct IlluminationAwareKDTreeVRAMUsage
{
	...
	std::size_t nisml_representative_ready		   = 0;
	...
	std::size_t get_total_bytes() const
	{
		return nodes + node_bounds + node_count + active_guiding_nodes + active_guiding_node_count + needs_split + guiding_distribution_count +
			   ...
			   nisml_cache + nisml_representative_sample_counts + nisml_representative_write_locks + nisml_representative_ready + nisml_cache_ready +
			   nisml_pending_cell_count + tree_cut_sampling_probabilities + tree_cut_sampling_cdfs + history_per_cell_sample_count +
