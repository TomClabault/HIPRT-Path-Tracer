...
public:
	...
	static void resize_common_buffers(GPURenderer* renderer, int new_width, int new_height,
		...
		OrochiBuffer<float>& decoupled_shading_reuse_mis_weights)
	{
		resize_decoupled_shading_reuse_buffer<IsReSTIRGI>(renderer, new_width, new_height, 
			decoupled_shading_reuse_buffer, decoupled_shading_reuse_mis_weights);
		...
		OrochiBuffer<float>& decoupled_shading_reuse_mis_weights)
	{
		...
		if (buffer_needed)
		{
			...
			decoupled_shading_reuse_mis_weights.resize((spatial_pass_settings.reuse_neighbor_count + 1) * new_width * new_height);
		}
	}
		...
		OrochiBuffer<float>& decoupled_shading_reuse_mis_weights)
	{
		...
		render_data_updated |= pre_render_update_decoupled_shading_buffers<IsReSTIRGI>(render_data, renderer, 
			decoupled_shading_buffer, decoupled_shading_reuse_mis_weights);
		...
		OrochiBuffer<float>& decoupled_shading_reuse_mis_weights)
	{
		...
		if (decoupled_shading_reuse_used)
		{
			...
			if (decoupled_shading_reuse_mis_weights.get_element_count() == 0)
			{
				int spatial_sample_count = ReSTIRSettingsHelper::get_restir_spatial_pass_settings<IsReSTIRGI>(render_data).reuse_neighbor_count + 1;

				decoupled_shading_reuse_mis_weights.resize(spatial_sample_count * renderer->m_render_resolution.x * renderer->m_render_resolution.y);
			}
		}
		else
		{
			...
			if (decoupled_shading_reuse_mis_weights.get_element_count() > 0)
				decoupled_shading_reuse_mis_weights.free();
		...
		OrochiBuffer<float>& decoupled_shading_reuse_mis_weights)
	{
		...
		render_data_invalidated |= free_decoupled_shading_reuse_buffer<IsReSTIRGI>(decoupled_shading_reuse_buffer, decoupled_shading_reuse_mis_weights);
		...
		OrochiBuffer<float>& decoupled_shading_reuse_mis_weights)
	{
		...
		common_spatial_pass_settings.decoupled_shading_reuse_mis_weights = decoupled_shading_reuse_mis_weights.get_device_pointer();
	}
};
