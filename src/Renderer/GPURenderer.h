...
public:
	 ...
	 *
	 * The 'delta_time' parameter should be how much time passed, in milliseconds, since the last
	 * call to pre_frame_render_update()
	 */
...
private:
	...
	// ---- Functions called by the pre_frame_render_update() method ----
	//
	...
	// If true, the buffer pointers of m_render_data will be updated when pre_frame_render_update() is called.
	...
	// without having called pre_frame_render_update() before
	...
	// Used to calculate the average luminance of each pixel for adaptive sampling
	OrochiBuffer<float> m_pixels_luminance_buffer;
	...
	OrochiBuffer<int> m_pixels_converged_sample_count_buffer;
	...
	// These values are updated when the pre_frame_render_update() is called
