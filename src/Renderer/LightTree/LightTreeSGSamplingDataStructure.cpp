											   ...
											   const std::vector<int>& triangles_vertex_indices)
{
	...
	ThreadManager::start_thread(ThreadManager::RENDERER_COMPUTE_LIGHT_TREE_SG,
								 ...
								 &vertices_positions]()
								{
									...
									int configured_tree_cut_size = m_light_tree_builder_sg.get_tree_cut_size();
									if (use_learning_to_cluster)
										m_light_tree_builder_sg.set_tree_cut_size(IlluminationAwareKDTreeInitialLightCutSize);
									...
									if (use_learning_to_cluster)
										m_light_tree_builder_sg.set_tree_cut_size(configured_tree_cut_size);
