...
struct LightTreeSGBuilderDeviceData
{
	void free()
	{
		if (m_device_nodes_buffer.size() == 0 && m_device_spatial_lobes_buffer.size() == 0)
			return;
	...
	{
		return m_device_nodes_buffer.get_byte_size() + m_device_spatial_lobes_buffer.get_byte_size() + m_device_indices_array_buffer.get_byte_size() +
