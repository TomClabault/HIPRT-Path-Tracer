...
GPUBakerKernel::GPUBakerKernel(std::shared_ptr<GPURenderer> renderer,
...
void GPUBakerKernel::bake_internal(int3_t bake_resolution, const void* bake_settings_pointer, int nb_kernel_iterations, std::string output_filename)
{
	m_bake_complete = false;

	struct ThreadData
	{
		int3_t bake_resolution;
	...
	ThreadManager::start_thread(
							"kernel_bake_" + m_kernel_title,
							[this, thread_data, nb_kernel_iterations]
							{
								OROCHI_CHECK_ERROR(oroCtxSetCurrent(m_renderer->get_hiprt_orochi_ctx()->orochi_ctx));

								int3_t& bake_resolution			  = thread_data->bake_resolution;
								...
								{
									g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_INFO, "%s",
															("Compiling " + m_kernel_title + " kernel...").c_str());
								...
								for (int i = 0; i < nb_kernel_launch; i++)
								{
									...
									m_bake_kernel.launch_asynchronous_3D(tile_size.x, tile_size.y, tile_size.z, bake_resolution.x, bake_resolution.y,
																		 bake_resolution.z, bake_kernel_launch_args, m_bake_stream);
								}
								...
								g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_INFO, "%s",
														(m_kernel_title + " completed in " + std::to_string(kernel_duration) + unit_suffix).c_str());
								...
								if (bake_resolution.z > 1)
								{
									...
									{
										Image32Bit image = Image32Bit(baked_data.data() + i * bake_resolution.x * bake_resolution.y, bake_resolution.x,
																	  bake_resolution.y, /* nb channels */ 1);
