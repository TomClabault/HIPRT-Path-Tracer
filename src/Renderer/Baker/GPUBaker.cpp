...
GPUBaker::GPUBaker(std::shared_ptr<GPURenderer> renderer) : m_renderer(renderer)
{
	...
	m_ggx_conductor_directional_albedo_bake_kernel = GPUBakerKernel(m_renderer, m_bake_stream, m_compiler_priority_mutex,
																	DEVICE_KERNELS_DIRECTORY "/Baking/GGXConductorDirectionalAlbedo.h",
																	"GGXConductorDirectionalAlbedoBake", "GGX conductor directional albedo");

	m_ggx_fresnel_directional_albedo_bake_kernel = GPUBakerKernel(m_renderer, m_bake_stream, m_compiler_priority_mutex,
																  DEVICE_KERNELS_DIRECTORY "/Baking/GGXFresnelDirectionalAlbedo.h",
																  "GGXFresnelDirectionalAlbedoBake", "GGX fresnel directional albedo");

	m_glossy_dielectric_directional_albedo_bake_kernel = GPUBakerKernel(m_renderer, m_bake_stream, m_compiler_priority_mutex,
																		DEVICE_KERNELS_DIRECTORY "/Baking/GlossyDielectricDirectionalAlbedo.h",
																		"GlossyDielectricDirectionalAlbedoBake", "dielectric directional albedo");

	m_ggx_glass_entering_directional_albedo_bake_kernel =
							GPUBakerKernel(m_renderer, m_bake_stream, m_compiler_priority_mutex, DEVICE_KERNELS_DIRECTORY "/Baking/GGXGlassDirectionalAlbedo.h",
										   "GGXGlassDirectionalAlbedoBakeEntering", "GGX glass directional albedo 1/2");
	m_ggx_glass_exiting_directional_albedo_bake_kernel =
							GPUBakerKernel(m_renderer, m_bake_stream, m_compiler_priority_mutex, DEVICE_KERNELS_DIRECTORY "/Baking/GGXGlassDirectionalAlbedo.h",
										   "GGXGlassDirectionalAlbedoBakeExiting", "GGX glass directional albedo 2/2");

	m_ggx_thin_glass_directional_albedo_bake_kernel = GPUBakerKernel(m_renderer, m_bake_stream, m_compiler_priority_mutex,
																	 DEVICE_KERNELS_DIRECTORY "/Baking/GGXThinGlassDirectionalAlbedo.h",
																	 "GGXThinGlassDirectionalAlbedoBake", "GGX thin glass directional albedo");
}
...
{
	m_ggx_conductor_directional_albedo_bake_kernel.bake_internal(make_int3(bake_settings.texture_size_cos_theta, bake_settings.texture_size_roughness, 1),
