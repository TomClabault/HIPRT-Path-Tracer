...
void GPUBaker::bake_ggx_hemispherical_albedo(const GGXHemisphericalAlbedoSettings& bake_settings, const std::string& output_filename)
{
	...
	ThreadManager::start_thread("bake_ggx_hemispherical_albedo_bake_start", [this, &bake_settings, &output_filename] {
		...
		m_ggx_hemispherical_albedo_bake_complete = true;
	});
...
void GPUBaker::bake_ggx_glass_hemispherical_albedo(const GGXGlassHemisphericalAlbedoSettings& bake_settings, const std::string& output_filename)
{
	m_ggx_glass_hemispherical_albedo_bake_complete = false;

	// Starting everything on a thread to avoid blocking to UI (during the compilation
	// of the kernel mainly)
	ThreadManager::start_thread("bake_ggx_glass_hemispherical_albedo_bake_start", [this, &bake_settings, &output_filename] {
		std::string kernel_filepath = DEVICE_KERNELS_DIRECTORY "/Baking/GGXGlassHemisphericalAlbedo.h";
		std::string kernel_function = "GGXGlassHemisphericalAlbedoBake";

		OROCHI_CHECK_ERROR(oroCtxSetCurrent(m_renderer->get_hiprt_orochi_ctx()->orochi_ctx));

		if (!m_ggx_glass_hemispherical_albedo_bake_kernel.has_been_compiled())
		{
			g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_INFO, "Compiling GGX glass hemispherical directional albedo kernel...");
			...
			std::lock_guard<std::mutex> lock(m_compiler_priority_mutex);
			m_renderer->take_kernel_compilation_priority();
			m_ggx_glass_hemispherical_albedo_bake_kernel = GPUKernel(kernel_filepath, kernel_function);
			m_ggx_glass_hemispherical_albedo_bake_kernel.compile(m_renderer->get_hiprt_orochi_ctx());
			m_renderer->release_kernel_compilation_priority();
		}

		m_ggx_glass_hemispherical_albedo_bake_buffer.resize(bake_settings.texture_size_cos_theta_o * bake_settings.texture_size_roughness * bake_settings.texture_size_ior);
		m_ggx_glass_hemispherical_albedo_bake_buffer_inverse.resize(bake_settings.texture_size_cos_theta_o * bake_settings.texture_size_roughness * bake_settings.texture_size_ior);

		// Forcefully dropping the const because launch_args wants non-const arguments even though
		// they are never going to be modified
		GGXGlassHemisphericalAlbedoSettings& bake_settings_non_const = const_cast<GGXGlassHemisphericalAlbedoSettings&>(bake_settings);
		HIPRTRenderData& render_data = m_renderer->get_render_data();
		float* device_buffer = m_ggx_glass_hemispherical_albedo_bake_buffer.get_device_pointer();
		float* device_buffer_inverse = m_ggx_glass_hemispherical_albedo_bake_buffer_inverse.get_device_pointer();

		void* launch_args[] = { &render_data, &bake_settings_non_const, &device_buffer, &device_buffer_inverse };

		g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_INFO, "Launching GGX glass hemispherical directional albedo baking...");
		m_ggx_glass_hemispherical_albedo_bake_kernel.launch_asynchronous_3D(4, 4, 4, bake_settings.texture_size_cos_theta_o, bake_settings.texture_size_roughness, bake_settings.texture_size_ior, launch_args, m_bake_stream);

		OROCHI_CHECK_ERROR(oroStreamSynchronize(m_bake_stream));
			
		g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_INFO, "GGX glass hemispherical directional albedo bake completed!");

		// Writing results to disk
		std::vector<float> baked_data = m_ggx_glass_hemispherical_albedo_bake_buffer.download_data();
		...
		std::vector<float> baked_data_inverse = m_ggx_glass_hemispherical_albedo_bake_buffer_inverse.download_data();
		for (int i = 0; i < bake_settings.texture_size_ior; i++)
		{
		...
		m_ggx_glass_hemispherical_albedo_bake_buffer.free();
		m_ggx_glass_hemispherical_albedo_bake_buffer_inverse.free();
		m_ggx_glass_hemispherical_albedo_bake_complete = true;
	});

	ThreadManager::detach_threads("bake_ggx_glass_hemispherical_albedo_bake_start");
}

bool GPUBaker::is_ggx_glass_hemispherical_albedo_bake_complete() const
{
	return m_ggx_glass_hemispherical_albedo_bake_complete;
}
