...
GPUBaker::GPUBaker(std::shared_ptr<GPURenderer> renderer) : m_renderer(renderer) 
{
	...
	m_ggx_conductor_directional_albedo_bake_kernel = GPUBakerKernel(m_renderer, m_bake_stream, m_compiler_priority_mutex,
		DEVICE_KERNELS_DIRECTORY "/Baking/GGXDirectionalAlbedo.h", "GGXDirectionalAlbedoBake", "GGX conductors directional albedo");
...
void GPUBaker::bake_ggx_conductor_directional_albedo(const GGXDirectionalAlbedoSettings& bake_settings, const std::string& output_filename)
{
