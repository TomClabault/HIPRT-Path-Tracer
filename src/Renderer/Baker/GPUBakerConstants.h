...
struct GPUBakerConstants
{
	...
	static std::string get_GGX_conductor_directional_albedo_texture_filename(
	...
	static std::string get_GGX_fresnel_directional_albedo_texture_filename(
							GGXMaskingShadowingFlavor masking_shadowing_term,
							...
							int texture_size_ior	   = GPUBakerConstants::GGX_FRESNEL_DIRECTIONAL_ALBEDO_TEXTURE_SIZE_IOR)
	{
		...
		return "GGX_Fresnel_" + flavor_string + std::to_string(texture_size_cos_theta) + "x" + std::to_string(texture_size_roughness) + "x" +
			   std::to_string(texture_size_ior) + ".hdr";
	}

	static std::string get_glossy_dielectric_directional_albedo_texture_filename(
							GGXMaskingShadowingFlavor masking_shadowing_term,
							...
							int texture_size_ior	   = GPUBakerConstants::GLOSSY_DIELECTRIC_TEXTURE_SIZE_IOR)
	{
		...
		return "Glossy_Ess_" + flavor_string + std::to_string(texture_size_cos_theta) + "x" + std::to_string(texture_size_roughness) + "x" +
			   std::to_string(texture_size_ior) + ".hdr";
	}

	static std::string get_GGX_glass_directional_albedo_texture_filename(
							GGXMaskingShadowingFlavor masking_shadowing_term,
							...
							int texture_size_ior	   = GPUBakerConstants::GGX_GLASS_DIRECTIONAL_ALBEDO_TEXTURE_SIZE_IOR)
	{
		...
		return "GGX_Glass_Ess_" + flavor_string + std::to_string(texture_size_cos_theta) + "x" + std::to_string(texture_size_roughness) + "x" +
			   std::to_string(texture_size_ior) + ".hdr";
	}

	static std::string get_GGX_thin_glass_directional_albedo_texture_filename(
							GGXMaskingShadowingFlavor masking_shadowing_term,
							...
							int texture_size_ior	   = GPUBakerConstants::GGX_THIN_GLASS_DIRECTIONAL_ALBEDO_TEXTURE_SIZE_IOR)
	{
		...
		return "GGX_Thin_Glass_Ess_" + flavor_string + std::to_string(texture_size_cos_theta) + "x" + std::to_string(texture_size_roughness) + "x" +
			   std::to_string(texture_size_ior) + ".hdr";
	}

	static std::string get_GGX_glass_directional_albedo_inv_texture_filename(
							GGXMaskingShadowingFlavor masking_shadowing_term,
							int texture_size_cos_theta = GPUBakerConstants::GGX_GLASS_DIRECTIONAL_ALBEDO_TEXTURE_SIZE_COS_THETA_O,
							int texture_size_roughness = GPUBakerConstants::GGX_GLASS_DIRECTIONAL_ALBEDO_TEXTURE_SIZE_ROUGHNESS,
							int texture_size_ior	   = GPUBakerConstants::GGX_GLASS_DIRECTIONAL_ALBEDO_TEXTURE_SIZE_IOR)
