...
def sample_concentric_disc_batch(b, n):
    r1 = torch.rand((b, n), dtype=torch.float, device='cpu') * 2.0 - 1.0
    r2 = torch.rand((b, n), dtype=torch.float, device='cpu') * 2.0 - 1.0
    ...
    zeros = torch.zeros((b, n, 2), dtype=torch.float, device='cpu')
...
def sample_cosine_batch(b, n):
    ...
    z = torch.tensor(1, dtype=torch.float, device='cpu') - xx - yy
    z = torch.sqrt(torch.where(z < 0., torch.tensor(0., dtype=torch.float, device='cpu'), z))
...
def sample_anisotropic_batch(b, n, v, alphax, alphay):
    ...
    zero = torch.tensor(0.0, dtype=torch.float, device='cpu')
    
    v = torch.unsqueeze(v, dim=1)
    v = v.repeat(1, n, 1)
    v[:, :, 2] = torch.maximum(zero, v[:, :, 2])

    # alphax = torch.tensor(alphax, dtype=torch.float, device='cpu')
    alphax = torch.unsqueeze( torch.unsqueeze(alphax, dim=1), dim=1)
    alphax = alphax.repeat(1, n, 1)
    # alphay = torch.tensor(alphay, dtype=torch.float, device='cpu')
    alphay = torch.unsqueeze( torch.unsqueeze(alphay, dim=1), dim=1)
    alphay = alphay.repeat(1, n, 1)
    
    u1 = torch.rand((b, n, 1), dtype=torch.float, device='cpu')
    u2 = torch.rand((b, n, 1), dtype=torch.float, device='cpu')
    ...
    zero_tensor = torch.zeros((vh.shape[0], vh.shape[1], 1), dtype=torch.float, device='cpu')

    T1 = torch.cat( (-vh[:, :, 1:2], vh[:, :, 0:1], zero_tensor), dim=2 ) / torch.sqrt(lensq)
    T1[idx] = torch.tensor([1.0, 0.0, 0.0], dtype=torch.float, device='cpu')
...
def buffer_rejection(b, n, buffer, target, v, alphax, alphay):
    ...
    eta = torch.rand((wi_target.shape[0], wi_target.shape[1], 1), dtype=torch.float, device='cpu')
...
def sample_anisotropic(n, v, alphax, alphay):
    ...
    zero_tensor = torch.tensor(0.0, dtype=torch.float, device='cpu')
    ...
    u1 = torch.rand((n, 1), dtype=torch.float, device='cpu')
    u2 = torch.rand((n, 1), dtype=torch.float, device='cpu')
    ...
    T1 = torch.hstack( (-vh[:, 1:2], vh[:, 0:1], torch.zeros((vh.shape[0], 1), dtype=torch.float, device='cpu')) ) / torch.sqrt(lensq)
    T1[idx] = torch.tensor([1.0, 0.0, 0.0], dtype=torch.float, device='cpu')
...
def rejection_sampling(n, target, v, alphax, alphay):
    ...
    eta = torch.rand((wi_target.shape[0], 1), dtype=torch.float, device='cpu')
...
def sample_concentric_disc(n):
    r1, r2 = torch.rand(n, dtype=torch.float, device='cpu') * 2.0 - 1.0, torch.rand(n, dtype=torch.float, device='cpu') * 2.0 - 1.0

    zero_x_y = torch.where(r1 == 0, True, False)
    zero_x_y = torch.logical_and(zero_x_y, torch.where(r2 == 0, True, False))
    zero_x_y = torch.stack((zero_x_y, zero_x_y), dim=1)
    zeros = torch.zeros((n, 2), dtype=torch.float, device='cpu')
