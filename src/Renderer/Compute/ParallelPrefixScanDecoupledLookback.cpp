...
{
	m_block_scan_kernel.set_kernel_file_path(DEVICE_KERNELS_DIRECTORY "/Compute/ParallelPrefixScanDecoupledLookback/Scan.h");
	m_block_scan_kernel.set_kernel_function_name("ParallelPrefixScanDecoupledLookback_Scan");
...
void ParallelPrefixScanDecoupledLookback::scan()
{
	...
	{
		g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_ERROR, "ParallelPrefixScanDecoupledLookback: scan() called but the class hasn't been setup properly or no data uploaded");
	...
	m_block_scan_kernel.launch_asynchronous(PARALLEL_PREFIX_SCAN_CHUNK_SIZE, 1, m_size, 1, block_scan_args, m_stream);
...
void ParallelPrefixScanDecoupledLookback::unit_test(std::shared_ptr<HIPRTOrochiCtx> hiprt_ctx, oroStream_t stream)
{
	...
	for (int i = 0; i < 100; i++)
	{
		...
		unsigned int test_size = rng() % (500000000);// PARALLEL_PREFIX_SCAN_CHUNK_SIZE * 256 * 256);
		...
		g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_INFO, "\tParallelPrefixScan unit test %d: scanned %u elements in %.3f ms. %.3fGItems/s", i, test_size, elapsed_time_ms / repeats, test_size / (elapsed_time_ms * 1e6f / repeats));
		...
		for (long long int j = 0; j < test_size; j++)
		{
			...
			{
				g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_ERROR, "ParallelPrefixScanDecoupledLookback unit test failed for test %d at index %d (size=%u): got %u, expected %u", i, j, test_size, output[j], expected_output[j]);
				break;
			}
		}
	}
