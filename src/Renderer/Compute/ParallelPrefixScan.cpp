...
void ParallelPrefixScan::init(std::shared_ptr<HIPRTOrochiCtx> hiprt_ctx, oroStream_t stream)
{
...
void ParallelPrefixScan::upload_input_data(const std::vector<unsigned int>& data)
{
	if (data.size() > 0xFFFFFFFFull)
	{
		g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_ERROR,
								"ParallelPrefixScan: input data size is too large (max supported size is 4,294,967,295 elements)");
	...
	unsigned int padded_divided_once_size =
							((divided_once_size + PARALLEL_PREFIX_SCAN_CHUNK_SIZE - 1) / PARALLEL_PREFIX_SCAN_CHUNK_SIZE) * PARALLEL_PREFIX_SCAN_CHUNK_SIZE;

	unsigned int divided_twice_size = padded_divided_once_size / PARALLEL_PREFIX_SCAN_CHUNK_SIZE;
	unsigned int padded_divided_twice_size =
							((divided_twice_size + PARALLEL_PREFIX_SCAN_CHUNK_SIZE - 1) / PARALLEL_PREFIX_SCAN_CHUNK_SIZE) * PARALLEL_PREFIX_SCAN_CHUNK_SIZE;

	unsigned int divided_thrice_size = padded_divided_twice_size / PARALLEL_PREFIX_SCAN_CHUNK_SIZE;
	unsigned int padded_divided_thrice_size =
							((divided_thrice_size + PARALLEL_PREFIX_SCAN_CHUNK_SIZE - 1) / PARALLEL_PREFIX_SCAN_CHUNK_SIZE) * PARALLEL_PREFIX_SCAN_CHUNK_SIZE;
...
void ParallelPrefixScan::scan()
{
	...
	{
		g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_ERROR,
								"ParallelPrefixScan: scan() called but the class hasn't been setup properly or no data uploaded");
	...
	else
	{
		...
		// We have scanned the input chunks, we need to scan the block sums now
		{
			...
			m_block_scan_kernel.launch_asynchronous(PARALLEL_PREFIX_SCAN_CHUNK_SIZE / 2, 1, (level_0_block_sums_size + 1) / 2, 1, scan_block_sums_args,
													m_stream);
		}
		...
		if (m_hierarchy_levels_used > 1)
		{
			// We now need to scan the sums of level 1
			{
				...
				m_block_scan_kernel.launch_asynchronous(PARALLEL_PREFIX_SCAN_CHUNK_SIZE / 2, 1, (level_1_block_sums_size + 1) / 2, 1, scan_block_sums_args,
														m_stream);
			}
			...
			{
				{
					...
					m_block_scan_kernel.launch_asynchronous(PARALLEL_PREFIX_SCAN_CHUNK_SIZE / 2, 1, PARALLEL_PREFIX_SCAN_CHUNK_SIZE / 2, 1,
															scan_block_sums_args, m_stream);
				}

				{
					...
					m_block_increment_kernel.launch_asynchronous(PARALLEL_PREFIX_SCAN_CHUNK_SIZE, 1, scanned_level_1_blocks_sums_size, 1, increment_args,
																 m_stream);
				}
			}
...
void ParallelPrefixScan::unit_test(std::shared_ptr<HIPRTOrochiCtx> hiprt_ctx, oroStream_t stream)
{
	...
	for (int i = 0; i < 100; i++)
	{
		unsigned int input_size_original = std::max((unsigned int)PARALLEL_PREFIX_SCAN_CHUNK_SIZE,
													static_cast<unsigned int>(PARALLEL_PREFIX_SCAN_CHUNK_SIZE * (rng() % 100u)));

		unsigned int padded_size = ((input_size_original + PARALLEL_PREFIX_SCAN_CHUNK_SIZE - 1) / PARALLEL_PREFIX_SCAN_CHUNK_SIZE) *
								   PARALLEL_PREFIX_SCAN_CHUNK_SIZE;
	...
	for (int i = 0; i < 100; i++)
	{
		...
		// 21GItems to beat
		g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_INFO, "\tParallelPrefixScan unit test %d: scanned %u elements in %.3f ms. %.3f GItems/s", i,
								test_size, elapsed_time_ms / repeats, (float)test_size / (elapsed_time_ms * 1e6f / repeats));
...
#pragma omp parallel for
		for (long long int j = 0; j < test_size; j++)
		{
			...
			{
				g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_ERROR,
										"ParallelPrefixScan unit test failed for test %d at index %lld (size=%u): got %u, expected %u", i, j, test_size,
