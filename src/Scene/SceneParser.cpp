...
void SceneParser::parse_scene_file(std::string scene_filepath, Assimp::Importer& assimp_importer, Scene& parsed_scene, SceneParserOptions& options)
{
	...
	scene = assimp_importer.ReadFile(scene_filepath, aiPostProcessSteps::aiProcess_PreTransformVertices | aiPostProcessSteps::aiProcess_Triangulate |
	...
	if (scene == nullptr)
	{
		...
		scene		   = assimp_importer.ReadFile(CommandlineArguments::DEFAULT_SCENE, aiPostProcessSteps::aiProcess_PreTransformVertices |
	...
	if (scene->mNumMaterials > NestedDielectricsInteriorStack::MAX_MATERIAL_INDEX)
	{
		g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_ERROR,
								"This scene contains too many materials for the renderer. Maximum number of material is: %d",
								NestedDielectricsInteriorStack::MAX_MATERIAL_INDEX);
	...
	for (int mesh_index = 0; mesh_index < scene->mNumMeshes; mesh_index++)
	{
		...
		parsed_scene.vertices_positions.insert(parsed_scene.vertices_positions.end(), reinterpret_cast<hiprtFloat3*>(&mesh->mVertices[0]),
	...
	ThreadManager::start_thread(ThreadManager::SCENE_LOADING_PARSE_EMISSIVE_TRIANGLES, ThreadFunctions::load_scene_parse_emissive_triangles, scene,
								std::ref(parsed_scene));
	ThreadManager::start_thread(ThreadManager::SCENE_LOADING_COMPUTE_TRIANGLE_AREAS, ThreadFunctions::load_scene_compute_triangle_areas,
								std::ref(parsed_scene));
}
...
void SceneParser::prepare_textures(const aiScene* scene,
								   std::vector<std::pair<aiTextureType, std::string>>& texture_paths,
								   std::vector<ParsedMaterialTextureIndices>& material_texture_indices,
								   std::vector<int>& material_indices,
								   std::vector<int>& texture_per_mesh,
...
void SceneParser::assign_material_texture_indices(std::vector<CPUMaterial>& materials,
												  ...
												  const std::vector<int>& material_textures_offsets)
{
	for (int material_index = 0; material_index < material_tex_indices.size(); material_index++)
	{
		...
		renderer_material.roughness_metallic_texture_index +=
			renderer_material.roughness_metallic_texture_index == MaterialConstants::NO_TEXTURE ? 0 : tex_index_offset;
		...
		renderer_material.specular_transmission_texture_index +=
			renderer_material.specular_transmission_texture_index == MaterialConstants::NO_TEXTURE ? 0 : tex_index_offset;
...
void SceneParser::read_material_properties(aiMaterial* mesh_material, CPUMaterial& renderer_material)
{
	...
	renderer_material.ior = 1.1f;
	if (renderer_material.specular_transmission == 1.0f && hippt::abs(renderer_material.base_color.luminance() - 0.8f) < 1.0e-2f)
		renderer_material.dielectric_priority = 2;

	renderer_material.make_safe();
}

std::vector<std::pair<aiTextureType, std::string>> SceneParser::get_textures_paths_and_indices(aiMaterial* mesh_material,
																							   ParsedMaterialTextureIndices& texture_indices)
{
	...
	if (roughness_index != MaterialConstants::NO_TEXTURE && metallic_index != MaterialConstants::NO_TEXTURE &&
		texture_paths[roughness_index].second == texture_paths[metallic_index].second)
	{
		...
		texture_paths[texture_indices.base_color_texture_index].second == texture_paths[texture_indices.normal_map_texture_index].second)
	{
		...
		g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_WARNING,
								"Material \"%s\" uses its base color texture as a normal map (texture is: %s); This is not supported and normal mapping will "
								"be disabled for this material.",
								mesh_material->GetName().C_Str(), texture_paths[texture_indices.base_color_texture_index].second.c_str());
	}
