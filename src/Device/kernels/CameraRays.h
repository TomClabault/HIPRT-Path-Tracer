...
HIPRT_DEVICE void reset_render(const HIPRTRenderData& render_data, uint32_t pixel_index)
{
	...
	PathSamplingStrategy == PATH_SAMPLING_RESTIR_PT
		...
		for (unsigned int cache_slot = 0; cache_slot < RESTIR_PT_SPMIS_REUSE_CELL_CACHE_SIZE; cache_slot++)
			render_data.render_settings.restir_pt_settings.spmis_settings
				.all_pixels_reuse_cell_pixel_index[pixel_index * RESTIR_PT_SPMIS_REUSE_CELL_CACHE_SIZE + cache_slot] =
...
#endif // #ifdef __KERNELCC__
{
	...
	Xorshift32Generator random_number_generator(render_data.get_input_random_seed(pixel_index));
	...
	if (intersection_found)
	{
		...
		//          If we don't have a primary hit, we're never going to use the float3_t in the 'primary_hit_position'
	...
	SSBN_update_screen_space_hash_grid(render_data, x, y, closest_hit_info.inter_point, closest_hit_info.geometric_normal);
