...
#endif
{
	...
	while (thread_index < number_of_cells_alive)
	{
		...
		unsigned int hash_grid_cell_index =
								number_of_cells_alive == regir_settings.get_total_number_of_cells_per_grid(primary_hit)
														? cell_alive_index
														: regir_settings.get_hash_cell_data_soa(primary_hit).grid_cells_alive_list[cell_alive_index];

		Xorshift32Generator random_number_generator(wang_hash(hash_grid_cell_index * ( render_data.render_settings.sample_number + 1)));
		...
		for (int i = 0; i < regir_settings.get_grid_fill_settings(primary_hit).get_non_canonical_reservoir_count_per_cell(); i++)
		{
			...
			ReGIRReservoir non_canonical_reservoir =
									regir_settings.get_cell_non_canonical_reservoir_from_index(hash_grid_cell_index, primary_hit, i, &invalid_sample);
			...
			light_sample.light_source_normal			= triangle_load_normal_not_normalized(render_data, light_sample.emissive_triangle_global_index);
			light_sample.light_area						= hippt::length(light_sample.light_source_normal) * 0.5f;
			light_sample.light_source_normal /= light_sample.light_area * 2.0f;
			...
			float non_canonical_target_function = ReGIR_grid_fill_evaluate_non_canonical_target_function(
									render_data, surface, primary_hit, light_sample.emission, light_sample.light_source_normal, light_sample.point_on_light,
		...
		regir_settings.get_non_canonical_pre_integration_factor_buffer(primary_hit)[hash_grid_cell_index] +=
								non_canonical_cell_integration_sum /
		...
		for (int i = 0; i < regir_settings.get_grid_fill_settings(primary_hit).get_canonical_reservoir_count_per_cell(); i++)
		{
			...
			light_sample.light_source_normal = triangle_load_normal_not_normalized(render_data, canonical_reservoir.sample.emissive_triangle_global_index);
			light_sample.light_area			 = hippt::length(light_sample.light_source_normal) * 0.5f;
			light_sample.light_source_normal /= light_sample.light_area * 2.0f;
			...
			float canonical_target_function = ReGIR_grid_fill_evaluate_canonical_target_function(render_data, surface, primary_hit, light_sample.emission,
		...
		regir_settings.get_canonical_pre_integration_factor_buffer(primary_hit)[hash_grid_cell_index] +=
								canonical_cell_integration_sum / (regir_settings.get_grid_fill_settings(primary_hit).get_canonical_reservoir_count_per_cell()) /
								render_data.render_settings.DEBUG_REGIR_PRE_INTEGRATION_ITERATIONS;
