    ...
    Xorshift32Generator& rng)
{
    ...
    for (int light_sample_index = 0; light_sample_index < regir_settings.get_grid_fill_settings(primary_hit).light_sample_count_per_cell_reservoir; light_sample_index++)
    {
        ...
        {
            light_sample = sample_one_emissive_triangle_with_cell_light_distribution(render_data, surface.cell_point, surface.cell_normal, hash_grid_cell_index, primary_hit, rng);
        ...
        else
        {
            ...
            float simple_strategy_PDF = pdf_of_emissive_triangle_hit_area_measure<ReGIR_GridFillCellDistributionsCanonicalSamplingTechnique>(render_data, surface.cell_point, hippt::normalize(render_data.current_camera.position - surface.cell_point), surface.cell_normal, 
                material,
