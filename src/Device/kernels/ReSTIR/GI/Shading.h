...
// - With everything specular at IOR 1.0f but roughness 1.0f, there's basically no bias. Even though the specular layer has no effect because of IOR 1.0f. So if
// the roughness of an inexistant layer changes the bias, it can only be a PDF issue?
...
//     - artificially fixing the proba of sampling the specular to 90 % and diffuse 10 % increases the bias quiiite a lot(but still converges correctly without
//     ReSTIR and when reusing 0 neighbor).Even more so when approaching 100 % specular(but not quite 100 %)
//     - sampling the specular & diffuse lobe based on the fresnel reflectance yields a different bias vs.sampling 50 / 50 (or 90 / 10).
//     - resampling more spatial neighbors makes things a bit worse.But only up to a certain point.For example, 6 spatial reuse passes with 16 candidates
//     each(which is huge) is barely worse than 1 spatial pass @ 16 candidates
//     - with 0 spatial neighbor reuse, it converges correctly, no matter the BSDF / sampling probas / ...
//     - there seems to be no bias with only 1 bounce(i.e.with paths, being at most : "camera -> first hit -> second hit").Bias only comes in with # of bounce
//     >= 2
// -------------------------- WHAT WE KNOW --------------------------
//
// -------------------------- DIRTY FIX RIGHT NOW --------------------------
...
GLOBAL_KERNEL_SIGNATURE(void) __launch_bounds__(64) ReSTIR_GI_Shading()
...
#endif // #ifdef __KERNELCC__
{
	...
	if (render_data.render_settings.enable_direct_lighting)
	{
		...
		camera_outgoing_radiance += estimate_direct_lighting</* deferred_BSDF_MIS */ false>(render_data, ray_payload, closest_hit_info, view_direction, x, y,
																							trash_context, random_number_generator);
	}
	...
	if (render_data.render_settings.restir_gi_settings.debug_view == ReSTIRGIDebugView::GI_FINAL_RESERVOIR_UCW)
		...
		path_tracing_accumulate_color(render_data, pixel_index,
									  ColorRGB32F(resampling_reservoir.confidence) * render_data.render_settings.restir_gi_settings.debug_view_scale_factor);
			 ...
			 render_data.render_settings.restir_gi_settings.common_spatial_pass.per_pixel_spatial_reuse_radius != nullptr)
	{
		float radius_percentage = (render_data.render_settings.restir_gi_settings.common_spatial_pass.per_pixel_spatial_reuse_radius[pixel_index] /
								   (float)render_data.render_settings.restir_gi_settings.common_spatial_pass.reuse_radius);
			 ...
			 render_data.render_settings.restir_gi_settings.common_spatial_pass.per_pixel_spatial_reuse_radius != nullptr)
	{
		unsigned char accepted_directions =
			hippt::popc(ReSTIRSettingsHelper::get_spatial_reuse_direction_mask_ull<ReSTIR_VARIANT_GI>(render_data, pixel_index));
...
#endif // #ifndef KERNELS_RESTIR_GI_SHADING_H
