...
#endif
{
	...
	if (!render_data.render_settings.restir_gi_settings.common_spatial_pass.use_hammersley)
	{
		...
		//if (hippt::popc(render_data.render_settings.restir_gi_settings.common_spatial_pass.current_pixel_directions_reuse_mask) <= 2 || render_data.render_settings.restir_gi_settings.common_spatial_pass.reuse_radius == 0)
		if (render_data.render_settings.restir_gi_settings.common_spatial_pass.reuse_radius == 0)
		{
			//hippt::atomic_fetch_add(&render_data.render_settings.DEBUG_SUM_COUNT[1], 1ull);
	...
	if (render_data.render_settings.restir_gi_settings.common_spatial_pass.do_neighbor_rotation && render_data.render_settings.restir_gi_settings.common_spatial_pass.use_hammersley)
		...
		// Disabling the rotation if not using the rotation setting or if not using Hammersley (because only hammersley needs rotation)
		rotation_theta = 0.0f;
