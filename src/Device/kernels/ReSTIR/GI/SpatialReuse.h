...
#endif
{
	...
	ReSTIRGIReservoir* input_reservoir_buffer = render_data.render_settings.restir_gi_settings.initial_candidates.initial_candidates_buffer;
	...
	count_valid_spatial_neighbors<true>(render_data,
		render_data.render_settings.restir_gi_settings.common_spatial_pass, render_data.render_settings.restir_gi_settings.neighbor_similarity_settings, 
	...
	for (int neighbor_index = start_index; neighbor_index < reused_neighbors_count + 1; neighbor_index++)
	{
		...
		if (neighbor_index < reused_neighbors_count && reused_neighbors_count > 32)
			...
			if (!check_neighbor_similarity_heuristics(render_data, render_data.render_settings.restir_gi_settings.neighbor_similarity_settings, neighbor_pixel_index, center_pixel_index, center_pixel_surface.shading_point, center_pixel_surface.shading_normal))
				continue;

		ReSTIRGIReservoir neighbor_reservoir = input_reservoir_buffer[neighbor_pixel_index];
		float target_function_at_center = 0.0f;

		bool do_neighbor_target_function_visibility = true;// do_include_visibility_term_or_not(render_data, neighbor_index);
		...
		if (target_function_at_center > 0.0f && neighbor_reservoir.UCW > 0.0f && neighbor_index != reused_neighbors_count)// && !(neighbor_reservoir.sample.flags & ReSTIRDISampleFlags::ReSTIR_GI_FLAGS_ENVMAP_SAMPLE))
		{
