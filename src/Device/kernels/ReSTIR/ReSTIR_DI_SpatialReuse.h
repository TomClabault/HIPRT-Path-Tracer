...
#define USE_GENERALIZED_BALANCE_HEURISTICS 0
...
HIPRT_HOST_DEVICE HIPRT_INLINE float ReSTIR_DI_evaluate_target_function(const HIPRTRenderData& render_data, const ReservoirSample& sample, const RendererMaterial& material, float3 view_direction, float3 shading_point, float3 shading_normal)
{
	...
	float3 to_light_direction = sample.point_on_light_source - evaluated_point;
	to_light_direction /= (distance_to_light = hippt::length(to_light_direction));

	float cosine_at_light_source = hippt::abs(hippt::dot(sample.light_source_normal, -to_light_direction));
	float cosine_at_evaluated_point = hippt::dot(to_light_direction, shading_normal);
	...
	if (cosine_at_evaluated_point > 0.0f)
	{
		...
		float geometry_term = cosine_at_evaluated_point * cosine_at_light_source / (distance_to_light * distance_to_light);
		target_function = (cosine_at_evaluated_point * bsdf_color * sample.emission * geometry_term).luminance();
		...
		if (target_function > 0.0f)
		{
			...
			bool visibility = !evaluate_shadow_ray(render_data, shadow_ray, distance_to_light);

			target_function *= visibility;
		}
	}
...
#endif
{
	...
	int center_pixel_index = x + y * res.x;
	float3 center_shading_point = render_data.g_buffer.first_hits[center_pixel_index];
	float3 center_shading_normal = render_data.g_buffer.shading_normals[center_pixel_index];
	float3 center_view_direction = render_data.g_buffer.view_directions[center_pixel_index];
	SimplifiedRendererMaterial center_material = render_data.g_buffer.materials[center_pixel_index];
	for (int i = 0; i < NEIGHBOR_REUSE_COUNT + 1; i++)
	{
		int neighbor_index = get_neighbor_pixel_index(i, neighbor_offsets, center_pixel_coords, res);
		if (neighbor_index == -1)
			continue;

		Reservoir neighbor_reservoir = render_data.aux_buffers.initial_reservoirs[neighbor_index];

		float target_function_at_center = ReSTIR_DI_evaluate_target_function(render_data, neighbor_reservoir.sample, center_material, center_view_direction, center_shading_point, center_shading_normal);

		// Evaluating the target function at all neighbors for the sample being resampled
#if USE_GENERALIZED_BALANCE_HEURISTICS
		// We already have the target function at the center pixel
		float nume = target_function_at_center;
		float denom = nume;

		for (int j = 0; j < NEIGHBOR_REUSE_COUNT; j++)
		{
			int neighbor_index_j = get_neighbor_pixel_index(j, neighbor_offsets, center_pixel_coords, res);
			if (neighbor_index_j == -1)
				continue;

			float3 neighbor_shading_point = render_data.g_buffer.first_hits[neighbor_index_j];
			...
			float target_function_at_j = ReSTIR_DI_evaluate_target_function(render_data, neighbor_reservoir.sample, neighbor_material, neighbor_view_direction, neighbor_shading_point, neighbor_shading_normal);
...
#else
		// 1/M weights
		float mis_weight = 1.0f / (NEIGHBOR_REUSE_COUNT + 1);
#endif

		out_reservoir.combine_with(neighbor_reservoir, mis_weight, target_function_at_center, random_number_generator);
	}

	out_reservoir.end();

	render_data.aux_buffers.spatial_reservoirs[center_pixel_index] = out_reservoir;
}
...
//	new_reservoir.debug_value = hippt::dot(hippt::normalize(new_reservoir.sample.point_on_light_source - center_pixel_shading_point), center_pixel_shading_normal);
