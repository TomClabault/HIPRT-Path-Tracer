												 ...
												 const ReSTIRPGSettings& restir_pg_settings)
{
	...
	VMF vmf				   = vmf_mixture_component.vmf;
	float component_weight = vmf_mixture_component.weight;

	return component_weight * vmf.density_evaluation(sample_direction);
}
...
#endif
{
	...
	for (int bounce = 0; bounce < render_data.render_settings.nb_bounces; bounce++)
	{
		...
		{
			/*if (hippt::is_pixel_index(11, 624))
				printf("Sample not valid at bounce %d\nSample variables: direction: %f %f %f, position: %f %f %f\n", bounce, sample.incident_direction.x,
					   sample.incident_direction.y, sample.incident_direction.z, sample.position.x, sample.position.y, sample.position.z);*/
		...
		{
			/*if (hippt::is_pixel_index(11, 624) || sample_hash_grid_index == 96906)
				printf("Grid cell not alive: %u\n", sample_hash_grid_index);*/
		...
		/*else if (hippt::is_pixel_index(11, 624))
			printf("Grid cell already alive: %u\n", sample_hash_grid_index);*/
