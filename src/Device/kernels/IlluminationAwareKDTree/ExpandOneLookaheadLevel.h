...
#endif
{
	...
	unsigned int current_frontier_count = *illumination_aware_kd_tree.current_frontier_count;
	if (frontier_index >= current_frontier_count)
		return;

	unsigned int parent_index			= illumination_aware_kd_tree.current_frontier[frontier_index];
	...
	if (parent.flags & IlluminationAwareKDTreeNodeFlag_HasChildren)
	{
		...
		illumination_aware_kd_tree.append_child_pair_to_frontier(illumination_aware_kd_tree.next_frontier, illumination_aware_kd_tree.next_frontier_count,
																 parent.left_child_index, parent.left_child_index + 1u);
	...
	unsigned int left_child = illumination_aware_kd_tree.reserve_physical_nodes(2u);
	...
	left_node.light_clustering_index	   = IlluminationAwareKDTreeNode::INVALID_LIGHT_CLUSTERING_INDEX;
	...
	illumination_aware_kd_tree.nodes[right_child]		= right_node;
