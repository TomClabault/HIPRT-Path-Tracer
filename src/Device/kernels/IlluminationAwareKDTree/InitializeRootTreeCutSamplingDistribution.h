...
#else
GLOBAL_KERNEL_SIGNATURE(void)
IlluminationAwareKDTree_InitializeRootTreeCutSamplingDistribution(IlluminationAwareKDTreeDevice illumination_aware_kd_tree, LightTreeSGDevice light_tree_sg)
#endif
{
#ifndef __KERNELCC__
	...
	for (unsigned int slot = 0; slot < tree_cut_size; slot++)
	{
		...
		illumination_aware_kd_tree.tree_cut_sampling_probabilities[distribution_slot] = probability;
		...
		illumination_aware_kd_tree.estimated_second_moment[distribution_slot] = 0.0f;
		illumination_aware_kd_tree.effective_sample_count[distribution_slot]  = 0.0f;
	...
	if (slot < tree_cut_size)
	{
		...
		illumination_aware_kd_tree.tree_cut_sampling_probabilities[distribution_slot] = probability;
		illumination_aware_kd_tree.tree_cut_sampling_cdfs[distribution_slot]		  = valid_slot ? exclusive_cdf : 0.0f;
		illumination_aware_kd_tree.estimated_second_moment[distribution_slot]		  = 0.0f;
		illumination_aware_kd_tree.effective_sample_count[distribution_slot]		  = 0.0f;
		illumination_aware_kd_tree.batch_second_moment_sum[distribution_slot]		  = 0.0f;
		illumination_aware_kd_tree.batch_sample_count[distribution_slot]			  = 0;
