...
inline IlluminationAwareKDTree_InitializeGlobalTreeCutPriorSamplingDistribution(IlluminationAwareKDTreeDevice kd_tree_device,
																				LightTreeSGDevice light_tree_sg,
																				[[maybe_unused]] int x)
#else
GLOBAL_KERNEL_SIGNATURE(void)
IlluminationAwareKDTree_InitializeGlobalTreeCutPriorSamplingDistribution(IlluminationAwareKDTreeDevice kd_tree_device, LightTreeSGDevice light_tree_sg)
#endif
{
#ifndef __KERNELCC__
	...
	unsigned int tree_cut_offset = kd_tree_device.nee_distributions.get_tree_cut_offset(guiding_distribution_index, tree_cut_size);
	...
	for (unsigned int slot = 0; slot < tree_cut_size; slot++)
	{
		...
		kd_tree_device.nee_distributions.tree_cut_sampling_probabilities[distribution_slot]				 = probability_u16;
		kd_tree_device.nee_distributions.tree_cut_sampling_cdfs[distribution_slot]						 = cdf_u16;
		kd_tree_device.nee_distributions.tree_cut_sampling_prior_pdfs[slot]								 = probability_u16;
		kd_tree_device.nee_distributions.tree_cut_sampling_prior_cdfs[slot]								 = cdf_u16;
		kd_tree_device.nee_distributions.history_per_cut_node_estimated_second_moment[distribution_slot] = 0.0f;
		kd_tree_device.nee_distributions.history_per_cut_node_sample_count[distribution_slot]			 = 0;
	...
	kd_tree_device.nee_distributions.history_per_cell_sample_count[guiding_distribution_index] = 0;
#else
	...
	if (slot < tree_cut_size)
	{
		...
		kd_tree_device.nee_distributions.tree_cut_sampling_probabilities[distribution_slot] = probability_u16;
		kd_tree_device.nee_distributions.tree_cut_sampling_cdfs[distribution_slot]			= static_cast<unsigned short int>(
		...
		kd_tree_device.nee_distributions.tree_cut_sampling_prior_pdfs[slot] = probability_u16;
		kd_tree_device.nee_distributions.tree_cut_sampling_prior_cdfs[slot] = static_cast<unsigned short int>(
			hippt::clamp(0.0f, 1.0f, valid_slot ? exclusive_cdf : 0.0f) * IlluminationAwareKDTreeNEELearntDistributionsDevice::U16_MAXIMUM_VALUE);
		kd_tree_device.nee_distributions.history_per_cut_node_estimated_second_moment[distribution_slot] = 0.0f;
		kd_tree_device.nee_distributions.history_per_cut_node_sample_count[distribution_slot]			 = 0;
		kd_tree_device.nee_distributions.batch_per_cut_node_second_moment_sum[distribution_slot]		 = 0.0f;
