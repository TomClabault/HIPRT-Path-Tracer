...
#ifndef KERNELS_NISML_OPTIMIZE_H
#define KERNELS_NISML_OPTIMIZE_H
...
#endif
{
	...
	if (thread_index < NeuralImportanceSamplingMLP::CONNECTIONS_COUNT)
	{
		...
		float mean							 = NISML_ADAM_BETA1 * mlp.adam_weights_means[thread_index] + (1.0f - NISML_ADAM_BETA1) * gradient;
		float variance						 = NISML_ADAM_BETA2 * mlp.adam_weights_variances[thread_index] + (1.0f - NISML_ADAM_BETA2) * gradient * gradient;
		...
		float updated_weight =
			mlp.connection_weights[thread_index] - mlp.adam_learning_rate * corrected_mean / (sqrtf(corrected_variance) + NISML_ADAM_EPSILON);
	...
	if constexpr (NeuralImportanceSamplingMLP::USE_BIASES)
	{
		if (thread_index < NeuralImportanceSamplingMLP::NEURON_COUNT)
		{
			...
			float mean							= NISML_ADAM_BETA1 * mlp.adam_biases_means[thread_index] + (1.0f - NISML_ADAM_BETA1) * gradient;
			float variance						= NISML_ADAM_BETA2 * mlp.adam_biases_variances[thread_index] + (1.0f - NISML_ADAM_BETA2) * gradient * gradient;
			mlp.adam_biases_means[thread_index] = mean;
			mlp.adam_biases_variances[thread_index] = variance;

			float corrected_mean	 = mean / beta1_correction;
			float corrected_variance = variance / beta2_correction;
			mlp.neurons_biases[thread_index] -= mlp.adam_learning_rate * corrected_mean / (sqrtf(corrected_variance) + NISML_ADAM_EPSILON);
		}
	}
}
