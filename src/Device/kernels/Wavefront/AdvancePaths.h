...
#ifndef KERNELS_WAVEFRONT_ADVANCE_PATHS_H
#define KERNELS_WAVEFRONT_ADVANCE_PATHS_H
...
extern "C"
{
	HIPRT_DEVICE __constant__ unsigned char WAVEFRONT_ADVANCE_RENDER_DATA[sizeof(HIPRTRenderData)];
}
GLOBAL_KERNEL_SIGNATURE(void)
__launch_bounds__(64) WavefrontAdvancePaths(unsigned int input_queue_index, unsigned int output_queue_index, unsigned int bounce_count)
#else  // #ifdef __KERNELCC__
GLOBAL_KERNEL_SIGNATURE(void)
inline WavefrontAdvancePaths(
	HIPRTRenderData render_data, unsigned int input_queue_index, unsigned int output_queue_index, unsigned int bounce_count, unsigned int queue_slot)
#endif // #ifdef __KERNELCC__
{
#ifdef __KERNELCC__
	HIPRTRenderData& render_data = *reinterpret_cast<HIPRTRenderData*>(WAVEFRONT_ADVANCE_RENDER_DATA);
	unsigned int queue_slot		 = blockIdx.x * blockDim.x + threadIdx.x;
#endif // #ifdef __KERNELCC__
	unsigned int input_count = hippt::atomic_fetch_add(render_data.wavefront_data.queue_counts[input_queue_index], 0u);
	if (queue_slot >= input_count)
		return;

	unsigned int pixel_index = render_data.wavefront_data.path_queues[input_queue_index][queue_slot];
...
#endif // #ifndef KERNELS_WAVEFRONT_ADVANCE_PATHS_H
