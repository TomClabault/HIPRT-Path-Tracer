 ...
 */
HIPRT_HOST_DEVICE void build_ONB(const float3& N, float3& T, float3& B)
{
 ...
 */
HIPRT_HOST_DEVICE void build_rotated_ONB(const float3& N, float3& T, float3& B, float basis_rotation)
{
 ...
 */
HIPRT_HOST_DEVICE float3 local_to_world_frame(const float3& N, const float3& V)
{
    float3 T, B;
    build_ONB(N, T, B);
...
HIPRT_HOST_DEVICE float3 local_to_world_frame(const float3& T, const float3& B, const float3& N, const float3& V)
{
    return hippt::normalize(V.x * T + V.y * B + V.z * N);
}
 ...
 */
HIPRT_HOST_DEVICE float3 world_to_local_frame(const float3& N, const float3& V)
{
    float3 T, B;
    build_ONB(N, T, B);
...
HIPRT_HOST_DEVICE float3 world_to_local_frame(const float3& T, const float3& B, const float3& N, const float3& V)
{
    return hippt::normalize(make_float3(hippt::dot(V, T), hippt::dot(V, B), hippt::dot(V, N)));
}
