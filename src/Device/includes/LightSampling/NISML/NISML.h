...
HIPRT_DEVICE unsigned int sample_nis_cluster(
	const float* residuals, const float* cluster_log_baseline_weights, unsigned int cluster_count, Xorshift32Generator& rng, float& out_cluster_probability)
{
	if (cluster_count == 0u)
	{
...
HIPRT_DEVICE unsigned int infer_and_sample_nis_cluster(const NeuralImportanceSamplingMLP& mlp,
													   const float* cluster_log_baseline_weights,
													   unsigned int cluster_count,
																			...
																			Xorshift32Generator& random_number_generator)
{
	...
	unsigned int cluster_count			= light_tree.settings.effective_tree_cut_size_neural_many_lights;
	...
	unsigned int selected_cluster_position =
		infer_and_sample_nis_cluster(render_data.nis_ml.mlp, cluster_log_baseline_weights, cluster_count, random_number_generator, shading_point,
									 view_direction, shading_normal, render_data, cluster_probability);
