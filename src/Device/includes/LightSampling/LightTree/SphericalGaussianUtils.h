...
HIPRT_DEVICE static float3_t GGX_dominant_visible_normal(const float3_t wi, const float2_t roughness)
{
	...
	const float2_t v = roughness * make_float2(wi.x, wi.y);
...
HIPRT_DEVICE static float SGGX(const float3_t m, const float2_t roughness)
{
	const float3_t stretched = make_float3(m.x / roughness.x, m.y / roughness.y, m.z);
...
HIPRT_DEVICE static float SGGX(const float3_t m, const float2x2 roughness_matrix)
{
	const float det = determinant(roughness_matrix);
	const float2x2 roughness_matrix_adjugate =
							float2x2(roughness_matrix.m[1][1], -roughness_matrix.m[0][1], -roughness_matrix.m[1][0], roughness_matrix.m[0][0]);
...
HIPRT_DEVICE static float SGGX_reflection_PDF(const float3_t wi, const float3_t m, const float2x2 roughness_matrix)
{
...
// Product of two SGs.
HIPRT_DEVICE static SGLobe SG_product(const float3_t axis1, const float sharpness1, const float3_t axis2, const float sharpness2)
{
	const float3_t axis	  = axis1 * sharpness1 + axis2 * sharpness2;
	const float sharpness = hippt::length(axis);

	const float3_t d		  = axis1 - axis2;
...
HIPRT_DEVICE static float upper_SG_clamped_cosine_integral_over_two_pi(const float sharpness)
{
	if (sharpness <= 0.5f)
		...
		return (((((((-1.0f / 362880.0f) * sharpness + 1.0f / 40320.0f) * sharpness - 1.0f / 5040.0f) * sharpness + 1.0f / 720.0f) * sharpness -
				  1.0f / 120.0f) * sharpness +
				 1.0f / 24.0f) * sharpness -
				1.0f / 6.0f) * sharpness +
			   0.5f;
...
HIPRT_DEVICE static float lower_SG_clamped_cosine_integral_over_two_pi(const float sharpness)
{
	...
	if (sharpness <= 0.5f)
		// Taylor-series approximation for the numerical stability.
		return e * (((((((((1.0f / 403200.0f) * sharpness - 1.0f / 45360.0f) * sharpness + 1.0f / 5760.0f) * sharpness - 1.0f / 840.0f) * sharpness +
						1.0f / 144.0f) * sharpness -
					   1.0f / 30.0f) * sharpness +
					  1.0f / 8.0f) * sharpness -
					 1.0f / 3.0f) * sharpness +
					0.5f);
...
HIPRT_DEVICE static float SG_clamped_cosine_product_integral_over_pi(const float cosine, const float sharpness)
{
	...
	const float lerp_factor = hippt::clamp(0.0f, 1.0f,
										   0.5f * (cosine * hippt::erfcf_fast(-tz) + hippt::erfcf_fast(t)) -
																   0.5f * INV_SQRTPI * hippt::intrin_expf(-tz * tz) *
																						   hippt::intrin_expm1f(t * t * (cosine * cosine - 1.0f)) / t);
