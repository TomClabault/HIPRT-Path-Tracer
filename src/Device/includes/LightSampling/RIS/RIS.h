			...
			if (light_sample_info.area_measure_pdf > 0.0f)
			{
				...
				if (cosine_at_evaluated_point > 0.0f && cosine_at_light_source > 1.0e-6f)
				{
					...
					float mis_weight			= balance_heuristic(
						   solid_angle_light_pdf, nb_light_candidates * DirectLightIntegrationFactor<DirectLightSamplingStrategy>(), bsdf_pdf, nb_bsdf_candidates);
																						 ...
																						 Xorshift32Generator& random_number_generator)
{
	...
	// Whether or not a BSDF sample has been retained by the reservoir
	// -1 BSDF candidate because we'er going to one with the bounce ray of the main path so that's one less candidate to do here
	for (int i = 0; i < nb_bsdf_candidates - 1; i++)
	{
		float bsdf_sample_pdf  = 0.0f;
		float target_function  = 0.0f;
		float candidate_weight = 0.0f;
	...
	// Note that we don't end the reservoir here because we're going to add one more BSDF sample to it with the bounce ray of the main path in deferred NEE
	// BSDF MIS so this reservoir is not done yet
	return reservoir;
}
