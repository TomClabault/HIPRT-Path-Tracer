...
#ifndef DEVICE_INCLUDES_LIGHT_SAMPLING_RISLTC_RISLTC_RESERVOIR_H
#define DEVICE_INCLUDES_LIGHT_SAMPLING_RISLTC_RISLTC_RESERVOIR_H
...
#include <mutex>
static std::mutex risltc_log_mutex;
...
struct RISLTCReservoir
{
	HIPRT_DEVICE void add_one_candidate(RISLTCSample new_sample, float weight, Xorshift32Generator& random_number_generator)
	{
		M++;
		weight_sum += weight;
	...
	HIPRT_DEVICE void sanity_check(int2_t pixel_coords = make_int2(-1, -1))
	{
		...
		if (M < 0)
		{
			std::lock_guard<std::mutex> lock(risltc_log_mutex);
...
#endif // #ifndef DEVICE_INCLUDES_LIGHT_SAMPLING_RISLTC_RISLTC_RESERVOIR_H
