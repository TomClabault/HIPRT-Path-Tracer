...
HIPRT_DEVICE float3_t sample_point_on_triangle_uniform_area(
						float3_t vertex_A, float3_t edge_AB, float3_t edge_AC, float triangle_area, Xorshift32Generator& rng, float& out_point_pdf)
{
	float rand_1 = rng();
	float rand_2 = rng();
												   ...
												   float& out_point_pdf)
{
	...
	if (do_projected_solid_angle_sampling)
		...
		// stuff
		out_sample_point = sample_point_on_triangle_projected_solid_angle_peters_2021(render_data, vertex_A, vertex_B, vertex_C, normal, shading_point,
