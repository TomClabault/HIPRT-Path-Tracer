...
HIPRT_DEVICE float3_t ltc_transform_cosine_to_shading(const HIPRTRenderData& render_data, float cos_theta_v, float3_t direction_or_position, const DeviceUnpackedEffectiveMaterial& material, LTCLobe ltc_lobe)
{
...
HIPRT_DEVICE float3_t ltc_transform_shading_to_cosine(float3x3 matrix_inverse, float3_t direction_or_position)
{
	return matrix_inverse * direction_or_position;
}

HIPRT_DEVICE float3x3 ltc_transform_shading_to_cosine_read_matrix(const HIPRTRenderData& render_data, float cos_theta_v, float3_t direction_or_position, const DeviceUnpackedEffectiveMaterial& material, LTCLobe ltc_lobe)
{
	ColorRGBA32F ltc_params = read_ltc_params(render_data.bsdfs_data.ltcs_data.GGX_conductor_ltc_params, cos_theta_v, material, ltc_lobe);
...
HIPRT_DEVICE float3_t ltc_transform_shading_to_cosine(const HIPRTRenderData& render_data, float cos_theta_v, float3_t direction_or_position, const DeviceUnpackedEffectiveMaterial& material, LTCLobe ltc_lobe)
{
...
HIPRT_DEVICE float ltc_jacobian(const HIPRTRenderData& render_data, float cos_theta_v, float3_t sampled_direction_shading_space, const DeviceUnpackedEffectiveMaterial& material, LTCLobe ltc_lobe)
{
	ColorRGBA32F ltc_params = read_ltc_params(render_data.bsdfs_data.ltcs_data.GGX_conductor_ltc_params, cos_theta_v, material, ltc_lobe);
	...
	float3_t direction_cosine_space = ltc_matrix_inverse * sampled_direction_shading_space;
