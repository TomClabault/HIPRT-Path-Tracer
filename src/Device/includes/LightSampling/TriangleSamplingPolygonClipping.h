...
#ifndef DEVICE_INCLUDES_LIGHT_SAMPLING_TRIANGLE_SAMPLING_PROJECTED_SOLID_ANGLE_CLIPPING_H
...
/*! Returns the intersection of the line connecting the given two points with
	the plane z == 0.0f.*/
HIPRT_DEVICE float3_t iz0(float3_t lhs, float3_t rhs)
{
	float lerp_factor = lhs.z / (lhs.z - rhs.z);
	// Equivalent to the following but I have trust issues regarding the
	// stability of mix()
	// return make_float3(mix(lhs.xy, rhs.xy, lerp_factor), 0.0f);
	float2_t fmaed = hippt::fma(make_float2(lerp_factor), make_float2(rhs.x, rhs.y),
								hippt::fma(-make_float2(lerp_factor), make_float2(lhs.x, lhs.y), make_float2(lhs.x, lhs.y)));
...
/*! This function clips the given convex polygon with vertices in v to the
	...
	three and vertex_count + 1. If it is less than MAX_POLYGON_VERTEX_COUNT_PROJECTED_SOLID_ANGLE_SAMPLING,
...
HIPRT_DEVICE unsigned int clip_polygon(unsigned int vertex_count, float3_t v[MAX_POLYGON_VERTEX_COUNT_PROJECTED_SOLID_ANGLE_SAMPLING])
{
	// The vertex count after clipping
	unsigned int vc;
	// Encode the whole configuration into a single integer
	unsigned int bit_mask = vertex_count;
