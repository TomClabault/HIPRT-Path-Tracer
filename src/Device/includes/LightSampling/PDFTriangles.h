														 ...
														 float light_area)
{
	...
	else if constexpr (trianglePointSamplingStrategy == TRIANGLE_POINT_SAMPLING_STRATEGY_SOLID_ANGLE)
	{
		...
		float pdf_solid_angle = solid_angle_triangle_solid_angle_pdf_from_sampled_point(
								...
								ltc_lobe_probas(render_data, vertex_A, vertex_B, vertex_C, shading_point, view_direction, shading_normal, triangle_emission,
												material),
								material);
	...
	else if constexpr (trianglePointSamplingStrategy == TRIANGLE_POINT_SAMPLING_STRATEGY_PROJECTED_SOLID_ANGLE)
	{
		...
		if (do_projected_solid_angle_sampling)
		{
			...
			float pdf_solid_angle = projected_solid_angle_triangle_solid_angle_pdf(render_data, vertex_A, vertex_B, vertex_C, shading_point, view_direction,
																				   shading_normal, point_on_triangle,
																				   ltc_lobe_probas(render_data, vertex_A, vertex_B, vertex_C, shading_point,
																								   view_direction, shading_normal, triangle_emission, material),
																				   material);
															...
															float3_t to_light_direction)
{
	...
	float pdf_area_measure = pdf_of_emissive_triangle_hit_area_measure<lightSamplingStrategy>(
							render_data, shading_point, view_direction, shading_normal, material, shading_point + hit_distance * to_light_direction,
							light_surface_normal, emissive_triangle_global_index, light_area, light_emission);
															...
															float3_t to_light_direction)
{
	return pdf_of_emissive_triangle_hit_solid_angle<lightSamplingStrategy>(render_data, shading_point, view_direction, shading_normal, material,
																		   ...
																		   triangle_load_area(render_data, emissive_triangle_global_index), light_emission,
																		   light_surface_normal, hit_distance, to_light_direction);
}
