...
HIPRT_DEVICE float triangle_solid_angle(float3_t vertex_A_worldspace, float3_t vertex_B_worldspace, float3_t vertex_C_worldspace, float3_t shading_point)
{
	float3_t vertex_A_local = hippt::normalize(vertex_A_worldspace - shading_point);
																				   ...
																				   LTCLobe ltc_lobe)
{
	...
	float3_t vertex_A_local = world_to_local_frame_non_normalized(T, B, shading_normal, vertex_A - shading_point);
	float3_t vertex_B_local = world_to_local_frame_non_normalized(T, B, shading_normal, vertex_B - shading_point);
	float3_t vertex_C_local = world_to_local_frame_non_normalized(T, B, shading_normal, vertex_C - shading_point);
	...
	float2_t householder_yz = make_float2(polygon.vertex_dirs[0].y, polygon.vertex_dirs[0].z) * (1.0f / (hippt::abs(polygon.vertex_dirs[0].x) + 1.0f));
	...
	float3_t vertices[3] = { polygon.vertex_dirs[1], polygon.vertex_dirs[0], polygon.vertex_dirs[2] };
	...
	float2x2 bottom_right_minor = float2x2(hippt::fma(make_float2(-dot_householder_0), householder_yz, make_float2(vertices[0].y, vertices[0].z)),
																		 ...
																		 const DeviceUnpackedEffectiveMaterial& material)
{
	...
	out_pdf += solid_angle_triangle_solid_angle_pdf_internal(render_data, vertex_A_world_space, vertex_B_world_space, vertex_C_world_space, shading_point,
															 view_direction, shading_normal, sampled_dir_shading_space, ltc_lobe_probabilities, material,
															 LTCLobe::COAT_LOBE);
																		   ...
																		   const DeviceUnpackedEffectiveMaterial& material)
{
	...
	float3_t T, B;
	build_ONB_XZ_plane(shading_normal, T, B, view_direction);
	float3_t sampled_dir_shading_space = world_to_local_frame(T, B, shading_normal, point_on_light - shading_point);
																	   ...
																	   Xorshift32Generator& rng)
{
	...
	float3_t parameters		 = polygon.triangle_parameters;
	float3_t vertices[3]	 = { polygon.vertex_dirs[1], polygon.vertex_dirs[0], polygon.vertex_dirs[2] };
	...
	float2_t cos_sin	  = make_float2(hippt::intrin_cosf(0.5f * target_solid_angle), hippt::intrin_sinf(0.5f * target_solid_angle));
	float3_t offset		  = vertices[0] * (parameters.x * cos_sin.x - parameters.y * cos_sin.y) + vertices[2] * (parameters.z * cos_sin.y);
	float3_t new_vertex_2 = hippt::fma(2.0f * make_float3(hippt::dot(vertices[0], offset) / hippt::dot(offset, offset)), offset, -vertices[0]);

	// Now sample the line between vertex 1 and the newly created vertex 2
	float rand_2	  = rng();
	float s2		  = hippt::dot(vertices[1], new_vertex_2);
	float s			  = hippt::mix_fma(1.0f, s2, rand_2);
	...
	float3_t sampled_direction = hippt::normalize(hippt::fma(-t_normed, s2, s) * vertices[1] + t_normed * new_vertex_2);
