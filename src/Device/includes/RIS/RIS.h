    ...
    const RISReservoir& reservoir, Xorshift32Generator& random_number_generator)
{
    ...
    if (!in_shadow)
    {
        ...
        else
        {
            ...
            BSDFContext bsdf_context(view_direction, closest_hit_info.shading_normal, closest_hit_info.geometric_normal, shadow_ray_direction_normalized, incident_light_info, ray_payload.volume_state, false, ray_payload.material, ray_payload.bounce, 0.0f); // TODO accumulated-roughness
        ...
        if (light_sample_pdf > 0.0f)
        {
            ...
            if (cosine_at_evaluated_point > 0.0f && cosine_at_light_source > 1.0e-6f)
            {
                ...
                else
                {
                    ...
                    contributes_enough = bsdf_pdf <= 0.0f || check_minimum_light_contribution(render_data.render_settings.minimum_light_contribution, light_contribution / bsdf_pdf / light_sample_pdf);
                    if (!contributes_enough)
