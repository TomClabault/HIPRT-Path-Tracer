...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_coat_eval(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, const float3& local_view_direction, const float3& local_to_light_direction, const float3& local_halfway_vector, float incident_medium_ior, float& out_pdf)
{
    // The coat lobe is just a microfacet lobe
    return torrance_sparrow_GGX_eval(render_data, material.get_coat_roughness(), material.get_coat_anisotropy(), material.get_coat_ior(), incident_medium_ior, local_view_direction, local_to_light_direction, local_halfway_vector, out_pdf);
}

/**
 * The sampled direction is returned in the local shading frame of the basis used for 'local_view_direction'
 */
HIPRT_HOST_DEVICE HIPRT_INLINE float3 principled_coat_sample(const DeviceEffectiveMaterial& material, const float3& local_view_direction, Xorshift32Generator& random_number_generator)
{
    return microfacet_GGX_sample_reflection(material.get_coat_roughness(), material.get_coat_anisotropy(), local_view_direction, random_number_generator);
}

HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_sheen_eval(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, const float3& local_view_direction, const float3& local_to_light_direction, float& pdf, float& out_sheen_reflectance)
{
    return sheen_ltc_eval(render_data, material, local_to_light_direction, local_view_direction, pdf, out_sheen_reflectance);
}

HIPRT_HOST_DEVICE HIPRT_INLINE float3 principled_sheen_sample(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, const float3& local_view_direction, const float3& shading_normal, Xorshift32Generator& random_number_generator)
{
    return sheen_ltc_sample(render_data, material, local_view_direction, shading_normal, random_number_generator);
}

HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_metallic_eval(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, float roughness, float anisotropy, float incident_ior, const float3& local_view_direction, const float3& local_to_light_direction, const float3& local_half_vector, float& pdf)
{
    float HoL = hippt::clamp(1.0e-8f, 1.0f, hippt::dot(local_half_vector, local_to_light_direction));

    ColorRGB32F F_metal = adobe_f82_tint_fresnel(material.get_base_color(), material.get_metallic_F82(), material.get_metallic_F90(), material.get_metallic_F90_falloff_exponent(), HoL);
    ColorRGB32F F_thin_film = thin_film_fresnel(material, incident_ior, HoL);
    ColorRGB32F F = hippt::lerp(F_metal, F_thin_film, material.get_thin_film());
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_diffuse_eval(const DeviceEffectiveMaterial& material, const float3& local_view_direction, const float3& local_to_light_direction, float& pdf)
{
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_specular_fresnel(const DeviceEffectiveMaterial& material, float material_ior, float relative_specular_ior, float cos_theta_i)
{
    ...
    float layer_above_IOR = material_ior / relative_specular_ior;
    ...
    float material_thin_film = material.get_thin_film();
...
HIPRT_HOST_DEVICE HIPRT_INLINE float principled_specular_relative_ior(float material_ior, float material_coat, float material_coat_ior, float incident_medium_ior)
{
    ...
    float incident_layer_ior = hippt::lerp(incident_medium_ior, material_coat_ior, material_coat);
    float relative_ior = material_ior / incident_layer_ior;
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_specular_eval(
    const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material,
    float material_roughness, float material_anisotropy, float material_thin_film,
    float material_ior,
    float relative_ior, 
    const float3& local_view_direction, const float3& local_to_light_direction, const float3& local_half_vector, 
    float& pdf)
{
    ColorRGB32F F = principled_specular_fresnel(material, material_ior, relative_ior, hippt::dot(local_to_light_direction, local_half_vector));
    ...
    ColorRGB32F specular = torrance_sparrow_GGX_eval<0>(render_data, material_roughness, material_anisotropy, F, local_view_direction, local_to_light_direction, local_half_vector, pdf);
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_glass_eval(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, const ColorRGB32F& base_color, float roughness, float anisotropy, RayVolumeState& ray_volume_state, const float3& local_view_direction, const float3& local_to_light_direction, float& pdf)
{
    pdf = 0.0f;
    ...
    float dispersion_abbe_number = material.get_dispersion_abbe_number();
    float dispersion_scale = material.get_dispersion_scale();
    ...
    bool thin_walled = material.get_thin_walled();
    ...
    float thin_film = material.get_thin_film();
    ColorRGB32F F_thin_film;
    if (thin_film > 0.0f)
    ...
    float scaled_roughness = MaterialUtils::get_thin_walled_roughness(thin_walled, roughness, relative_eta);
    ...
    if (reflecting)
    {
        color = torrance_sparrow_GGX_eval<0>(render_data, scaled_roughness, anisotropy, F, local_view_direction, local_to_light_direction, local_half_vector, pdf);
    ...
    else
    {
        ...
        float alpha_x;
        float alpha_y;
        MaterialUtils::get_alphas(scaled_roughness, anisotropy, alpha_x, alpha_y);
        ...
        color = base_color * D * (ColorRGB32F(1.0f) - F) * G2 * hippt::abs(HoL * HoV / denom);
        if (thin_walled)
 ...
 * The sampled direction is returned in the local shading frame of the basis used for 'local_view_direction'
 */
HIPRT_HOST_DEVICE HIPRT_INLINE float3 principled_glass_sample(const DevicePackedTexturedMaterial* materials_buffer, const DeviceEffectiveMaterial& material, float roughness, float anisotropy, RayVolumeState& ray_volume_state, const float3& local_view_direction, Xorshift32Generator& random_number_generator)
{
 ...
 */
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_coat_compute_darkening(const DeviceEffectiveMaterial& material, const ColorRGB32F& base_color, float roughness, float relative_eta, float view_dir_fresnel)
{
    if (material.get_coat_darkening() == 0.0f)
    ...
    float K = hippt::lerp(Ks, Kr, roughness); // Eq. 68
    ...
    float sheen = material.get_sheen();
    ColorRGB32F base_albedo = (base_color + material.get_sheen_color() * sheen) / (1.0f + sheen);
    ...
    darkening = hippt::lerp(ColorRGB32F(1.0f), darkening, material.get_coat() * material.get_coat_darkening());
 ...
 */
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F internal_eval_coat_layer(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, 
    const ColorRGB32F& base_color, float material_roughness,
    ...
    float incident_ior, float coat_weight, bool refracting, float coat_proba, ColorRGB32F& layers_throughput, float& out_cumulative_pdf)
{
    ...
    float coat_ior = material.get_coat_ior();
    if (coat_weight > 0.0f || refracting)
    {
        ...
        if (!material.get_coat_medium_absorption().is_white())
        {
            ...
            ColorRGB32F coat_absorption = exp(-(ColorRGB32F(1.0f) - pow(sqrt(material.get_coat_medium_absorption()), traveled_distance_angle)) * material.get_coat_medium_thickness());
            layer_below_attenuation *= coat_absorption;
        }

        layer_below_attenuation *= principled_coat_compute_darkening(material, base_color, material_roughness, coat_ior / incident_ior, view_dir_fresnel);
        ...
        layer_below_attenuation = hippt::lerp(ColorRGB32F(1.0f), layer_below_attenuation, material.get_coat());
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F internal_eval_sheen_layer(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, const float3& local_view_direction, const float3& local_to_light_direction, const float3& world_space_to_light_direction, const float3& shading_normal, float incident_ior, float sheen_weight, float sheen_proba, ColorRGB32F& layers_throughput, float& out_cumulative_pdf)
{
    if (sheen_weight > 0.0f)
    {
        ...
        layers_throughput *= 1.0f - material.get_sheen() * sheen_reflectance;
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F internal_eval_metal_layer(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, 
                                                                     const float3& local_view_direction, const float3 local_to_light_direction, const float3& local_half_vector, 
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F internal_eval_glass_layer(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, 
    const ColorRGB32F& base_color, float roughness, float anisotropy,
    RayVolumeState& ray_volume_state, const float3& local_view_direction, const float3 local_to_light_direction, 
    float glass_weight, float glass_proba, ColorRGB32F& layers_throughput, float& out_cumulative_pdf)
{
    if (glass_weight > 0.0f)
    {
        float glass_pdf;
        ColorRGB32F contribution = principled_glass_eval(render_data, material,
            base_color, roughness, anisotropy,
 ...
 */
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_specular_compute_darkening(const DeviceEffectiveMaterial& material, const ColorRGB32F& base_color, float relative_eta, float view_dir_fresnel)
{
    if (material.get_specular_darkening() == 0.0f)
        return ColorRGB32F(1.0f);
    ...
    ColorRGB32F base_albedo = base_color;
    // This approximation of the amount of total internal reflection can then be used to
    // compute the darkening of the base caused by the clearcoating
    ColorRGB32F darkening = (1.0f - K) / (ColorRGB32F(1.0f) - base_albedo * K);
    darkening = hippt::lerp(ColorRGB32F(1.0f), darkening, material.get_specular() * material.get_specular_darkening());

    return darkening;
}

HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F internal_eval_specular_layer(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material,
    const ColorRGB32F& material_base_color,
    float material_roughness, float material_anisotropy, float material_thin_film, float material_coat_ior, 
    const float3& local_view_direction, const float3 local_to_light_direction, const float3& local_half_vector, const float3& shading_normal, 
    float incident_medium_ior, float specular_weight, float specular_proba, ColorRGB32F& layers_throughput, float& out_cumulative_pdf)
{
    ...
    {
        float relative_ior = principled_specular_relative_ior(material.get_ior(), material.get_coat(), material_coat_ior, incident_medium_ior);
        float base_ior = material.get_ior();

        float specular_pdf;
        ColorRGB32F contribution = principled_specular_eval(render_data, material, 
            material_roughness, material_anisotropy, material_thin_film, base_ior,
            relative_ior, local_view_direction, local_to_light_direction, local_half_vector, specular_pdf);
        ...
        if (hippt::abs(relative_ior - 1.0f) > 1.0e-3f)
        {
            ...
            contribution *= hippt::lerp(ColorRGB32F(1.0f), material.get_specular_tint() * material.get_specular_color(), material.get_specular());
            ...
            // the transmitted portion of light. That's actually either a good approximation or the
            // exact solution. That was shown in GDC 2017 [PBR Diffuse Lighting for GGX + Smith Microsurfaces]
            layer_below_attenuation *= ColorRGB32F(1.0f) - principled_specular_fresnel(material, base_ior, relative_ior, local_to_light_direction.z);
            ...
            // is the same, Fresnel is symmetrical. But because we don't have the exact direction reflected from the
            // base layer, we're using the view direction instead
            ColorRGB32F view_dir_fresnel = principled_specular_fresnel(material, base_ior, relative_ior, local_view_direction.z);
            ...
            layer_below_attenuation *= principled_specular_compute_darkening(material, material_base_color, relative_ior, view_dir_fresnel.luminance());
            ...
            layer_below_attenuation = hippt::lerp(ColorRGB32F(1.0f), layer_below_attenuation, material.get_specular());
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F internal_eval_diffuse_layer(const HIPRTRenderData& render_data, float incident_ior, const DeviceEffectiveMaterial& material, const float3& local_view_direction, const float3 local_to_light_direction, float diffuse_weight, float diffuse_proba, ColorRGB32F& layers_throughput, float& out_cumulative_pdf)
{
 ...
 */
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F internal_eval_glossy_base(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material,
                                                                     const ColorRGB32F& base_color,
                                                                     float material_roughness, float material_anisotropy,
                                                                     const float3& local_view_direction, const float3 local_to_light_direction, const float3& local_half_vector, 
                                                                     ...
                                                                     ColorRGB32F& layers_throughput, float& out_cumulative_pdf)
{
    ...
    glossy_base_contribution += internal_eval_specular_layer(render_data, material, 
        base_color, material_roughness, material_anisotropy, material.get_thin_film(), material.get_coat_ior(),
        local_view_direction_rotated, local_to_light_direction_rotated, local_half_vector_rotated, shading_normal, 
        incident_medium_ior, specular_weight, specular_proba_norm, layers_throughput, out_cumulative_pdf);
 ...
 */
HIPRT_HOST_DEVICE HIPRT_INLINE void principled_bsdf_get_lobes_weights(const DeviceEffectiveMaterial& material,
                                                                      bool outside_object,
                                                                      ...
                                                                      float& out_diffuse_weight, float& out_glass_weight)
{
    ...
    out_coat_weight = material.get_coat() * outside_object;
    out_sheen_weight = material.get_sheen() * outside_object;
    ...
    float second_roughness_weight = material.get_second_roughness_weight();
    ...
    float specular_transmission = material.get_specular_transmission();
    out_specular_weight = (1.0f - metallic) * (1.0f - specular_transmission) * material.get_specular() * outside_object;
 ...
 * vertex normals)
 */
HIPRT_HOST_DEVICE HIPRT_INLINE void principled_bsdf_get_lobes_weights_fringe_fix(const DeviceEffectiveMaterial& material, 
                                                                      ...
                                                                      float& out_diffuse_weight, float& out_glass_weight)
{
    out_outside_object = hippt::dot(view_direction, in_out_normal) > 0 || material.get_thin_walled();
    out_glass_weight = (1.0f - material.get_metallic()) * material.get_specular_transmission();
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_bsdf_eval(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, RayVolumeState& ray_volume_state, const float3& view_direction, float3 shading_normal, const float3& to_light_direction, float& pdf)
{
    ...
    bool outside_object = hippt::dot(view_direction, shading_normal) > 0 || material.get_thin_walled();
    ...
    float3 TR, BR;
    build_rotated_ONB(shading_normal, TR, BR, material.get_anisotropy_rotation() * M_PI);
    ...
    float anisotropy = material.get_anisotropy();
    final_color += internal_eval_coat_layer(render_data, material, 
        ...
        incident_medium_ior, coat_weight, refracting, coat_proba, layers_throughput, pdf);
    ...
    final_color += internal_eval_metal_layer(render_data, material, local_view_direction_rotated, local_to_light_direction_rotated, local_half_vector_rotated, roughness, anisotropy, incident_medium_ior, metal_1_weight * !refracting, metal_1_proba, layers_throughput, pdf);
    final_color += internal_eval_metal_layer(render_data, material, local_view_direction_rotated, local_to_light_direction_rotated, local_half_vector_rotated, material.get_second_roughness(), anisotropy, incident_medium_ior, metal_2_weight * !refracting, metal_2_proba, layers_throughput, pdf);
    ...
    // there
    final_color += internal_eval_glass_layer(render_data, material, base_color, roughness, anisotropy,
        ray_volume_state, local_view_direction_rotated, local_to_light_direction_rotated, 
        glass_weight, glass_proba, layers_throughput, pdf);
    final_color += internal_eval_glossy_base(render_data, material, base_color, roughness, anisotropy,
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F principled_bsdf_sample(const HIPRTRenderData& render_data, const DeviceEffectiveMaterial& material, RayVolumeState& ray_volume_state, const float3& view_direction, const float3& shading_normal, const float3& geometric_normal, float3& output_direction, float& pdf, Xorshift32Generator& random_number_generator)
{
    ...
    float3 TR, BR;
    build_rotated_ONB(normal, TR, BR, material.get_anisotropy_rotation() * M_PI);
    float3 local_view_direction_rotated = world_to_local_frame(TR, BR, normal, view_direction);

    if (rand_1 < cdf[0])
    {
        float3 TR_coat, BR_coat;
        build_rotated_ONB(normal, TR_coat, BR_coat, material.get_coat_anisotropy_rotation() * M_PI);
    ...
    else if (rand_1 < cdf[1])
    {
        ...
        output_direction = local_to_world_frame(TR, BR, normal, principled_metallic_sample(material.get_roughness(), material.get_anisotropy(), local_view_direction_rotated, random_number_generator));
    else if (rand_1 < cdf[3])
        // Second metallic lobe sample
        output_direction = local_to_world_frame(TR, BR, normal, principled_metallic_sample(material.get_second_roughness(), material.get_anisotropy(), local_view_direction_rotated, random_number_generator));
    else if (rand_1 < cdf[4])
        output_direction = local_to_world_frame(TR, BR, normal, principled_specular_sample(material.get_roughness(), material.get_anisotropy(), local_view_direction_rotated, random_number_generator));
        ...
        output_direction = local_to_world_frame(TR, BR, normal, principled_glass_sample(render_data.buffers.materials_buffer, material, material.get_roughness(), material.get_anisotropy(),
            ray_volume_state, local_view_direction_rotated, random_number_generator));
