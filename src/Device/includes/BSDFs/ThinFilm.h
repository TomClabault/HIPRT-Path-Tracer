...
HIPRT_HOST_DEVICE ColorRGB32F eval_sensitivity(float opd, float shift)
{
    // Use Gaussian fits
 ...
 */
HIPRT_HOST_DEVICE void fresnel_phase(float cos_theta_i,
    float eta1,
...
HIPRT_HOST_DEVICE void fresnel_conductor(float cos_theta_i,
    float eta, float k,
    float& Rp2, float& Rs2) 
{
 ...
 */
HIPRT_HOST_DEVICE ColorRGB32F RGB_hue_shift(const ColorRGB32F& color, float hue_shift_degrees)
{
 ...
 */
HIPRT_HOST_DEVICE ColorRGB32F thin_film_fresnel(const DeviceUnpackedEffectiveMaterial& material,
