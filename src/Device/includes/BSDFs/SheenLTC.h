...
#ifdef __KERNELCC__
	ltc_parameters_texture_pointer = &render_data.buffers.sheen_ltc_parameters_texture;
#else
	ltc_parameters_texture_pointer = render_data.buffers.sheen_ltc_parameters_texture;
#endif

	float2 parameters_uv = make_float2(1.0 - cos_theta, 1.0f - roughness);
 ...
 */
HIPRT_HOST_DEVICE HIPRT_INLINE float3 rotateVector(const float3& u, const float3& axis, float angle) 
{
	float sin_angle = sin(angle);
	float cos_angle = cos(angle);

	return u * cos_angle + axis * hippt::dot(u, axis) * (1.0f - cos_angle) + sin_angle * hippt::cross(axis, u);
}
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F sheen_ltc_eval(const HIPRTRenderData& render_data, const SimplifiedRendererMaterial& material, const float3& local_to_light_direction, const float3& local_view_direction, float& pdf)
{
	...
	float3 to_light_standard_frame = rotateVector(local_to_light_direction, make_float3(0, 0, 1), -phi);
