...
HIPRT_DEVICE static void get_blue_noise_texture_offset(unsigned int blue_noise_texture_width,
													   ...
													   unsigned int zero_based_sample_number,
													   int& out_offset_x,
													   int& out_offset_y)
{
	constexpr float golden_ratio_2d = 1.3247179572f;
	out_offset_x					= (int)(1.0f / golden_ratio_2d * (float)(blue_noise_texture_width - 1) * (zero_based_sample_number + 1));
	out_offset_y = (int)(1.0f / (golden_ratio_2d * golden_ratio_2d) * (float)(blue_noise_texture_height - 1) * (zero_based_sample_number + 1));
}
