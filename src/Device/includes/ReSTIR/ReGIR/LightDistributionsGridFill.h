...
HIPRT_DEVICE LightSampleInformation sample_one_emissive_triangle_with_cell_light_distribution(const HIPRTRenderData& render_data, float3 shading_point, unsigned int hash_grid_cell_index, bool primary_hit, Xorshift32Generator& rng)
{
    ...
    LightSampleInformation light_sample = sample_point_on_generic_triangle_and_fill_light_sample_information(render_data, shading_point, emissive_triangle_global_index, rng);
    if (light_sample.emissive_triangle_global_index == -1)
