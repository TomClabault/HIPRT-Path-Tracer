...
struct ReGIRReservoir
{
	...
	HIPRT_DEVICE bool stream_sample_raw(float mis_weight, float target_function, float source_pdf, int emissive_triangle_global_index, unsigned int point_on_light_random_seed, Xorshift32Generator& rng)
	{
		...
		if (rng() < resampling_weight / weight_sum)
		{
			...
			sample.point_on_light_random_seed = point_on_light_random_seed;
