    ...
    if (bsdf_sample_pdf > 0.0f)
    {
        ...
        if (intersection_found && !shadow_light_ray_hit_info.hit_emission.is_black() && bsdf_sample_pdf_area_measure > 0.0f)
        {
            ...
            if (mesh_index != -1)
            {
                ...
                mis_weight = balance_heuristic(bsdf_sample_pdf_area_measure, 1, PDF_light_distributions, regir_settings.shading_settings.number_of_neighbors, canonical_technique_pdf, ReGIR_GridFillCellDistributionsCanonicalSampleCount * DirectLightIntegrationFactor<DirectLightSamplingStrategy>());
            }
            else
