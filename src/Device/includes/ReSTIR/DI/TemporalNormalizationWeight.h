...
struct ReSTIRDITemporalNormalizationWeight<RESTIR_DI_BIAS_CORRECTION_1_OVER_Z>
{
		...
		float& out_normalization_nume, float& out_normalization_denom)
	{
		...
		float center_pixel_target_function = ReSTIR_DI_evaluate_target_function<ReSTIR_DI_SpatialReuseBiasUseVisiblity>(render_data, reservoir.sample, center_pixel_surface);
		...
		if (temporal_neighbor_M > 0)
		{
			...
			float temporal_neighbor_target_function = ReSTIR_DI_evaluate_target_function<ReSTIR_DI_SpatialReuseBiasUseVisiblity>(render_data, reservoir.sample, temporal_neighbor_surface);
...
struct ReSTIRDITemporalNormalizationWeight<RESTIR_DI_BIAS_CORRECTION_MIS_LIKE>
{
		...
		float& out_normalization_nume, float& out_normalization_denom)
	{
		...
		if (temporal_neighbor_M > 0)
			...
			temporal_neighbor_target_function = ReSTIR_DI_evaluate_target_function<ReSTIR_DI_SpatialReuseBiasUseVisiblity>(render_data, reservoir.sample, temporal_neighbor_surface);
