...
HIPRT_HOST_DEVICE HIPRT_INLINE float get_jacobian_determinant_reconnection_shift(const HIPRTRenderData& render_data, const float3& reconnection_point, const float3& reconnection_point_surface_normal, const float3& center_pixel_shading_point, const float3& neighbor_shading_point)
{
	...
	float3 to_reconnection_point_direction_from_center = reconnection_point - center_pixel_shading_point;
	float3 to_reconnection_point_direction_from_neighbor = reconnection_point - neighbor_shading_point;
	to_reconnection_point_direction_from_center /= (distance_to_reconnection_point_from_center = hippt::length(to_reconnection_point_direction_from_center));
	to_reconnection_point_direction_from_neighbor /= (distance_to_reconnection_point_from_neighbor = hippt::length(to_reconnection_point_direction_from_neighbor));

	float cosine_at_reconnection_point_from_center = hippt::abs(hippt::dot(-to_reconnection_point_direction_from_center, reconnection_point_surface_normal));
	float cosine_at_reconnection_point_from_neighbor = hippt::abs(hippt::dot(-to_reconnection_point_direction_from_neighbor, reconnection_point_surface_normal));
...
HIPRT_HOST_DEVICE HIPRT_INLINE void count_valid_spatial_neighbors(const HIPRTRenderData& render_data, 
																  const ReSTIRCommonSpatialPassSettings& spatial_pass_settings, const ReSTIRCommonNeighborSimiliaritySettings& neighbor_similarity_settings,
																  ...
																  int& out_valid_neighbor_count, int& out_valid_neighbor_M_sum, int& out_neighbor_heuristics_cache)
{
	...
	for (int neighbor_index = 0; neighbor_index < reused_neighbors_count; neighbor_index++)
	{
		...
		if (!check_neighbor_similarity_heuristics(render_data, neighbor_similarity_settings, neighbor_pixel_index, center_pixel_index, center_pixel_surface.shading_point, center_pixel_surface.shading_normal))
			continue;

		out_valid_neighbor_M_sum += ReSTIRDISettingsHelper::get_restir_spatial_pass_input_reservoir_M<IsReSTIRGI>(render_data, neighbor_pixel_index);
...
HIPRT_HOST_DEVICE HIPRT_INLINE int3 find_temporal_neighbor_index(const HIPRTRenderData& render_data,
																 const ReSTIRCommonTemporalPassSettings& temporal_pass_settings, const ReSTIRCommonNeighborSimiliaritySettings& neighbor_similarity_settings, 
																 const float3& current_shading_point, const float3& current_normal, int center_pixel_index, Xorshift32Generator& random_number_generator)
{
	...
	for (int i = 0; i < temporal_pass_settings.max_neighbor_search_count + 1; i++)
	{
		...
		if (check_neighbor_similarity_heuristics(render_data, neighbor_similarity_settings, temporal_neighbor_index, center_pixel_index, current_shading_point, current_normal, use_previous_frame_g_buffer))
