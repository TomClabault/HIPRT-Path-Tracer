...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F shade_ReSTIR_DI_reservoir_MIS_BSDF(const HIPRTRenderData& render_data,
    RayVolumeState& ray_volume_state, DeviceUnpackedEffectiveMaterial& material, int last_prim_index,
    const float3& shading_point, const float3& view_direction, const float3& shading_normal, const float3& geometric_normal,
    const ReSTIRDIReservoir& reservoir, Xorshift32Generator& random_number_generator)
{
    ...
    if (!in_shadow)
    {
        ...
        if (cosine_at_evaluated_point > 0.0f)
        {
            ...
            float mis_weight = balance_heuristic(1.0f / reservoir.UCW * area_measure_to_solid_angle_conversion, bsdf_pdf);
            final_color += mis_weight * bsdf_color * reservoir.UCW * sample_emission * cosine_at_evaluated_point * area_measure_to_solid_angle_conversion;
        }
    }

    float bsdf_sample_pdf;
    float3 sampled_direction;
    BSDFIncidentLightInfo incident_light_info_out;
    BSDFContext bsdf_sample_context(view_direction, shading_normal, geometric_normal, make_float3(0, 0, 0), incident_light_info_out, ray_volume_state, false, material, /* bounce. Always 0 for ReSTIR DI */ 0, 0.0f, MicrofacetRegularization::RegularizationMode::REGULARIZATION_MIS);
    ColorRGB32F bsdf_color = bsdf_dispatcher_sample(render_data, bsdf_sample_context, sampled_direction, bsdf_sample_pdf, random_number_generator);

    if (bsdf_sample_pdf > 0.0f)
    {
        hiprtRay new_ray;
        new_ray.origin = shading_point;
        new_ray.direction = sampled_direction;

        ShadowLightRayHitInfo shadow_light_ray_hit_info;
        bool intersection_found = evaluate_shadow_light_ray(render_data, new_ray, 1.0e35f, shadow_light_ray_hit_info, last_prim_index, 0, random_number_generator);

        // Checking that we did hit something and if we hit something,
        // it needs to be emissive
        if (intersection_found)
        {
            float light_pdf = pdf_of_emissive_triangle_hit_solid_angle(render_data, shadow_light_ray_hit_info, sampled_direction);
            float mis_weight = balance_heuristic(bsdf_sample_pdf, light_pdf);
            float cosine_term = hippt::abs(hippt::dot(shading_normal, sampled_direction));

            final_color += mis_weight * bsdf_color * cosine_term * shadow_light_ray_hit_info.hit_emission / bsdf_sample_pdf;
        }
    }
...
HIPRT_HOST_DEVICE HIPRT_INLINE ColorRGB32F sample_light_ReSTIR_DI_MIS_BSDF(const HIPRTRenderData& render_data, RayPayload& ray_payload, const HitInfo closest_hit_info, const float3& view_direction, Xorshift32Generator& random_number_generator, int2 pixel_coords)
{
    int pixel_index = pixel_coords.x + pixel_coords.y * render_data.render_settings.render_resolution.x;
    ...
    return shade_ReSTIR_DI_reservoir_MIS_BSDF(render_data,
        ray_payload.volume_state, ray_payload.material, closest_hit_info.primitive_index,
        closest_hit_info.inter_point, view_direction, closest_hit_info.shading_normal, closest_hit_info.geometric_normal,
        reservoir, random_number_generator);
}
