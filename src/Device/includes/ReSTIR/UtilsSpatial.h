	...
	else
	{
			...
			render_data.render_settings.sample_number >= render_data.render_settings.adaptive_sampling_min_samples)
		{
			...
			if (spatial_pass_settings.allow_converged_neighbors_reuse)
			{
				...
				Xorshift32Generator rng_converged_neighbor_reuse(render_data.get_updated_random_seed(
										center_pixel_coords.x + center_pixel_coords.y * render_data.render_settings.render_resolution.x));
