...
HIPRT_DEVICE unsigned int ReSTIR_spmis_hash(
	ReSTIRCommonSPMISSettings& spmis_settings, int pixel_x, int pixel_y, float3_t shading_point, float3_t surface_normal)
{
	return screen_space_gbuffer_hash(pixel_x, pixel_y, spmis_settings.tile_size, shading_point, surface_normal, 0.0f);
}
...
HIPRT_DEVICE void ReSTIR_spmis_insert_pixel_hash(HIPRTRenderData& render_data, int pixel_x, int pixel_y, float3_t shading_point, float3_t surface_normal)
{
	...
	unsigned int current_hash_value = spmis_settings.pixel_hashes[pixel_index];

	if (current_hash_value == HashGrid::UNDEFINED_CHECKSUM_OR_GRID_INDEX)
		spmis_settings.pixel_hashes[pixel_index] = ReSTIR_spmis_hash(spmis_settings, pixel_x, pixel_y, shading_point, surface_normal);
}
