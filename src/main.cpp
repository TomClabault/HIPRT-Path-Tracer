...
int main(int argc, char* argv[])
{
	...
	g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_INFO, "Scene geometry parsed in %ldms",
							std::chrono::duration_cast<std::chrono::milliseconds>(stop_scene - start_scene).count());
	...
	ThreadManager::start_thread(ThreadManager::ENVMAP_LOAD_FROM_DISK_THREAD, ThreadFunctions::read_envmap, std::ref(envmap_image),
								cmd_arguments.skysphere_file_path, 4, true);
#if GPU_RENDER
	...
	ParallelPrefixScanDecoupledLookback<unsigned int>::unit_test(hiprt_orochi_ctx, renderer->get_main_stream());

	parsed_scene.print_statistics(std::cout);

	stop_full = std::chrono::high_resolution_clock::now();
	g_imgui_logger.add_line(ImGuiLoggerSeverity::IMGUI_LOGGER_INFO, "Full scene parsed & built in %ldms",
							std::chrono::duration_cast<std::chrono::milliseconds>(stop_full - start_full).count());
	...
	stop_full = std::chrono::high_resolution_clock::now();
	std::cout << "Full scene & textures parsed in " << std::chrono::duration_cast<std::chrono::milliseconds>(stop_full - start_full).count() << "ms"
			  << std::endl;
	cpu_renderer.render();
