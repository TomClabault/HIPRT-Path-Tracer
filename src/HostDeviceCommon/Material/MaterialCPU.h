...
struct CPUMaterial
{
	...
	DevicePackedTexturedMaterial pack_to_GPU() const
	{
		...
		mat.set_retro_reflection(retro_reflection);
	...
	HIPRT_HOST_DEVICE void make_safe()
	{
		...
		retro_reflection		= hippt::clamp(0.0f, 1.0f, retro_reflection);
	...
	float global_emissive_factor = 0.0f; // This factor is baked into 'emission' before being sent to the GPU
