...
#define ENERGY_COMPENSATION_MODE_LUTS_TURQUIN	0
#define ENERGY_COMPENSATION_MODE_INVARIANCE_CUI 1
 ...
 *
 *  - GGX_VNDF_SAMPLING
 *		Sample the distribution of visible normals as proposed in [Sampling the GGX Distribution of Visible Normals, Heitz, 2018]
 *
 *  - GGX_VNDF_SPHERICAL_CAPS
 *		Sample the distribution of visible normals using spherical caps as proposed in [Sampling Visible GGX Normals with Spherical Caps, Dupuy & Benyoub, 2023]
 *
 *  - GGX_VNDF_BOUNDED [Not Yet Implemented]
 *		Sample the distribution of visible normals with a bounded VNDF sampling range as proposed in [Bounded VNDF Sampling for Smith-GGX Reflections, Eto &
 *		Tokuyoshi, 2023]
 *
 */
 ...
 *
 * Possible options are KERNEL_OPTION_TRUE and KERNEL_OPTION_FALSE
 */
#define PrincipledBSDFDoEnergyCompensation KERNEL_OPTION_TRUE

/**
 * If PrincipledBSDFDoEnergyCompensation is KERNEL_OPTION_TRUE, this option allows to choose how to ensure energy conservation for microfacet BRDFs in the
 * Principled BSDF.
 *
 *	- ENERGY_COMPENSATION_MODE_LUTS_TURQUIN
 *		Uses precomputed LUTs following the method of [Practical multiple scattering compensation for microfacet models, Turquin, 2019] to compensate energy
 *		loss due to multiple scattering in microfacet BRDFs. Quite fast but approximate
 *
 *	- ENERGY_COMPENSATION_MODE_INVARIANCE_CUI
 *		Uses an implementation of the method proposed in [Multiple-bounce Smith Microfacet BRDFs using the Invariance
...
/**
 * Whether or not to perform energy compensation (it's an approximation for the clearcoat layer, it's not perfect but very good in most cases) for the clearcoat
 * layer of the Principled BSDF
 */
...
/**
 * Whether or not to use multiple scattering to conserve energy and use a Fresnel compensation term i.e. account for Fresnel when light scatters multiple times
 * on the microsurface. This increases saturation and has a noticeable impact. Only applies to conductors. This term always is implicitely used for dielectrics
 *
 * This is implemented by following [Practical multiple scattering compensation for microfacet models, Turquin, 2019]
 *
...
/**
 * If this is true, then delta distribution lobes of the principled BSDF will not be evaluated if the incident light direction used for the evaluation doesn't
 * come from sampling the delta distribution lobe itself
 *
 ...
 *
 * This means that the diffuse layer will be sampled more often at normal incidence since this is where the specular layer reflects close to no light.
 *
 * At grazing angle however, where the specular layer reflects the most light (and so the diffuse layer below isn't reached by that light that is reflected by
 * the specular layer), it is the specular layer that will be sampled more often.
 */
...
#endif // #ifndef HOST_DEVICE_COMMON_PRINCIPLED_BSDF_KERNEL_OPTIONS_H
