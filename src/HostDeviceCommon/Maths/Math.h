...
namespace hippt
{
	...
	__device__ static float3 fma(float3 a, float3 b, float3 c)
	{
	 ...
	 * Returns the 'warpSize' runtime constant of the GPU
	 */
	static constexpr int warp_size() { return 1; }
	...
	static float3 cross(float3 u, float3 v) { return make_float3(u.y * v.z - u.z * v.y, u.z * v.x - u.x * v.z, u.x * v.y - u.y * v.x); }
	/*static float3 cross(hiprtFloat3 u, float3 v) { return make_float3(u.y * v.z - u.z * v.y, u.z * v.x - u.x * v.z, u.x * v.y - u.y * v.x); }
	static float3 cross(float3 u, hiprtFloat3 v) { return make_float3(u.y * v.z - u.z * v.y, u.z * v.x - u.x * v.z, u.x * v.y - u.y * v.x); }*/
	 ...
	 * Minimum of each component of the float3 against x
	 */
	static float3 min(float3 a, float x) { return make_float3(min(a.x, x), min(a.y, x), min(a.z, x)); }
	...
	static float3 normalize(float3 u) { return make_float3(u.x, u.y, u.z) / hippt::sqrt(hippt::dot(u, u)); }
